...
function flyToDestination(viewModel, destination) {
    ...
    if (!defined(promise)) {
        promise = computeFlyToLocationForCartographic(
            destination,
            terrainProvider,
        );
    }
...
function clearCredits(viewModel) {
    if (
        ...
        viewModel._previousCredits.forEach((credit) => {
            viewModel._scene.frameState.creditDisplay.removeStaticCredit(
                credit,
            );
        });
    }
...
async function updateSearchSuggestions(viewModel) {
    ...
    for (const service of viewModel._geocoderServices) {
        const newResults = await service.geocode(
            query,
            GeocodeType.AUTOCOMPLETE,
        );
