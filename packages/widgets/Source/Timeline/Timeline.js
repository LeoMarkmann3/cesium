...
Timeline.prototype.zoomTo = function (startTime, stopTime) {
    ...
    if (this._clock && this._clock.clockRange !== ClockRange.UNBOUNDED) {
        ...
        const endOffset = JulianDate.secondsDifference(
            clockEnd,
            this._endJulian,
        );
...
Timeline.prototype._makeTics = function () {
    ...
    if (ticIndex > 0) {
        while (ticIndex > 0) {
            ...
            --ticIndex;
            if (
                Math.abs(remainder(mainTic, timelineTicScales[ticIndex])) <
                0.00001
            ) {
        ...
        if (smallestIndex >= 0) {
            while (smallestIndex < ticIndex) {
                ...
                if (
                    Math.abs(
                        remainder(subTic, timelineTicScales[smallestIndex]),
                    ) < 0.00001 &&
        ...
        if (timeline._mouseMode === timelineMouseMode.scrub) {
            ...
            if (x < 0) {
                ...
                timeline._setTimeBarTime(
                    x,
                    (x * timeline._timeBarSecondsSpan) /
            ...
            if (dx !== 0) {
                const dsec =
                    (dx * timeline._timeBarSecondsSpan) /
                    timeline._topDiv.clientWidth;
                timeline.zoomTo(
                    JulianDate.addSeconds(
