...
describe(
    ...
    function () {
        ...
        afterEach(function () {
            geocoderViewModel =
                geocoderViewModel && geocoderViewModel.destroy();
        });
        ...
        it("moves camera when search command invoked", async function () {
            ...
            const promise = new Promise((resolve) => {
                destinationFoundCallback = async function (
                    viewModel,
                    destination,
                ) {
                    await GeocoderViewModel.flyToDestination(
                        viewModel,
                        destination,
                    );
                    resolve();
        ...
        it("automatic suggestions can be navigated by arrow up/down keys", function () {
            ...
            geocoderViewModel._searchText = "some_text";
            return GeocoderViewModel._updateSearchSuggestions(
                geocoderViewModel,
            ).then(function () {
                expect(geocoderViewModel._selectedSuggestion).toEqual(
                    undefined,
                );
                geocoderViewModel._handleArrowDown(geocoderViewModel);
                expect(
                    geocoderViewModel._selectedSuggestion.displayName,
                ).toEqual("a");
                geocoderViewModel._handleArrowDown(geocoderViewModel);
                geocoderViewModel._handleArrowDown(geocoderViewModel);
                expect(
