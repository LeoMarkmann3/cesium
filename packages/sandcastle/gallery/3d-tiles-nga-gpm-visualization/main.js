...
class App {
    ...
    set currentTileset(value) {
        ...
        this.notifyCurrentTilesetChanged(
            oldCurrentTileset,
            this.currentTileset,
        );
    }
    ...
    async createCurrentTileset(dataSetOption) {
        ...
        if (Cesium.defined(dataSetOption.initialCameraConfiguration)) {
            viewer.scene.camera.setView(
                dataSetOption.initialCameraConfiguration,
            );
        } else {
...
class PropertyTextureShaders {
    ...
    static createShaderOption(
        ...
        return {
            ...
            onselect: function () {
                if (
                    Cesium.defined(propertyName0) &&
                    Cesium.defined(propertyName1)
                ) {
                    app.currentCustomShader =
                        PropertyTextureShaders.createShader2D(
                            propertyName0,
                            sourceMin0,
                            sourceMax0,
                            propertyName1,
                    ...
                    app.currentCustomShader =
                        PropertyTextureShaders.createShader1D(
                            propertyName0,
                            sourceMin0,
                            sourceMax0,
                        );
...
class TexturePickingHandler {
    constructor() {
        ...
        handler.setInputAction(function (movement) {
            ...
            if (!Cesium.defined(worldPosition)) {
                ...
                const uncertainty = pickUncertaintyFromTexture(
                    movement.position,
                );
    ...
    updateErrorLabel(center, sizeX, sizeY) {
        ...
        const radialString =
            sizeX === 16.0 ? "(unknown)" : `${sizeX.toFixed(4)}m`;
...
class AnchorPointVisualizer {
        ...
        viewer.scene.postRender.addEventListener(function () {
            ...
            for (const anchorPoint of that.anchorPointEntities.keys()) {
                if (!that.anchorPointsForNextFrame.has(anchorPoint)) {
                    const anchorPointEntity =
                        that.anchorPointEntities.get(anchorPoint);
            ...
            for (const anchorPoint of that.anchorPointLabels.keys()) {
                if (!that.anchorPointsForNextFrame.has(anchorPoint)) {
                    const anchorPointLabel =
                        that.anchorPointLabels.get(anchorPoint);
    ...
    createAnchorPointLabel(anchorPointVisualizationData) {
        const standardDeviations =
            anchorPointVisualizationData.standardDeviations;
    ...
    updateForCurrentTilesetChanged(oldCurrentTileset, newCurrentTileset) {
        ...
        if (Cesium.defined(newCurrentTileset)) {
            newCurrentTileset.tileVisible.addEventListener(
                this.tileVisibleListener,
            );
        }
    }
...
async function updateModelFromView() {
    ...
    const anchorPointLabelsVisible = Boolean(
        viewModel.anchorPointLabelsVisible,
    );
