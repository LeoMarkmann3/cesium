...
DeferredLoading.prototype._process = function (isFirstCall) {
    ...
    while (defined(child)) {
        ...
        if (
            ...
            if (
                this._timeoutSet ||
                KmlDataSource._getTimestamp() >
                    this._started + this._timeThreshold
            ) {
                this._giveUpTime();
                return;
            }
...
function createDefaultLabel() {
    ...
    label.translucencyByDistance = new NearFarScalar(
        3000000,
        1.0,
        5000000,
        0.0,
...
function getIconHref(
    ...
    if (canRefresh) {
        ...
        const viewFormat =
            ...
            defaultViewFormat;
        const httpQuery = queryStringValue(
            iconNode,
            "httpQuery",
            namespaces.kml,
        );
        if (defined(viewFormat)) {
            hrefResource.setQueryParameters(
                queryToObject(cleanupString(viewFormat)),
            );
        }
        if (defined(httpQuery)) {
            hrefResource.setQueryParameters(
                queryToObject(cleanupString(httpQuery)),
            );
        }
...
function applyStyle(
    ...
    for (let i = 0, len = styleNode.childNodes.length; i < len; i++) {
        ...
        if (node.localName === "IconStyle") {
            ...
            if (
                defined(queryNumericValue(node, "physicalWidth", namespaces.gx))
            ) {
                oneTimeWarning(
            ...
            if (
                defined(
                    queryBooleanValue(node, "labelVisibility", namespaces.gx),
                )
            ) {
                oneTimeWarning(
            ...
            const bgColor =
                parseColorString(
                    queryStringValue(node, "bgColor", namespaces.kml),
                ) ?? Color.WHITE;
            const textColor =
                parseColorString(
                    queryStringValue(node, "textColor", namespaces.kml),
                ) ?? Color.BLACK;
...
function computeFinalStyle(
    ...
    if (styleIndex !== -1) {
        ...
        if (inlineStyleNode.localName === "Style") {
            ...
            for (let p = 0; p < pairs.length; p++) {
                ...
                if (key === "normal") {
                    const styleUrl = queryStringValue(
                        pair,
                        "styleUrl",
                        namespaces.kml,
                    );
                    if (defined(styleUrl)) {
                        ...
                        if (!defined(styleEntity)) {
                            styleEntity = styleCollection.getById(
                                `#${styleUrl}`,
                            );
                        }
                        if (defined(styleEntity)) {
    ...
    const externalStyle = queryStringValue(
        placeMark,
        "styleUrl",
        namespaces.kml,
    );
                    ...
                    if (key === "normal") {
                        ...
                        if (!defined(styleCollection.getById(id))) {
                            ...
                            if (defined(styleUrl)) {
                                ...
                                if (isExternal && defined(sourceResource)) {
                                    styleUrl =
                                        sourceResource.getUrlComponent() +
                                        styleUrl;
                                }
...
function processMultiTrack(
    ...
    for (let i = 0, len = trackNodes.length; i < len; i++) {
        ...
        for (let x = 0; x < length; x++) {
            const position = readCoordinate(
                coordNodes[x].textContent,
                ellipsoid,
            );
...
function processExtendedData(node, entity) {
    const extendedDataNode = queryFirstNode(
        node,
        "ExtendedData",
        namespaces.kml,
    );
    ...
    if (
        defined(queryFirstNode(extendedDataNode, "SchemaData", namespaces.kml))
    ) {
...
function processCamera(featureNode, entity, ellipsoid) {
    ...
    if (defined(camera)) {
        const lon =
            queryNumericValue(camera, "longitude", namespaces.kml) ?? 0.0;
        const lat =
            queryNumericValue(camera, "latitude", namespaces.kml) ?? 0.0;
        const altitude =
            queryNumericValue(camera, "altitude", namespaces.kml) ?? 0.0;

        const heading =
            queryNumericValue(camera, "heading", namespaces.kml) ?? 0.0;
...
function processLookAt(featureNode, entity, ellipsoid) {
    ...
    if (defined(lookAt)) {
        const lon =
            queryNumericValue(lookAt, "longitude", namespaces.kml) ?? 0.0;
        const lat =
            queryNumericValue(lookAt, "latitude", namespaces.kml) ?? 0.0;
        const altitude =
...
function processGroundOverlay(
    ...
    const zIndex = queryNumericValue(
        groundOverlay,
        "drawOrder",
        namespaces.kml,
    );
    if (defined(positions)) {
        ...
        if (defined(latLonBox)) {
            ...
            if (defined(south)) {
                south = CesiumMath.clampToLatitudeRange(
    ...
    if (defined(href)) {
        ...
        geometry.material = queryColorValue(
            groundOverlay,
            "color",
            namespaces.kml,
        );
...
function processNetworkLinkQueryString(
    ...
    if (defined(camera) && camera._mode !== SceneMode.MORPHING) {
        ...
        if (defined(centerCartesian)) {
            ...
            centerCartesian =
                ellipsoid.cartographicToCartesian(centerCartographic);
        }
            ...
            const promise = load(
                ...
                .then(function (rootElement) {
                    ...
                    if (
                        ...
                        const hasNetworkLinkControl =
                            defined(networkLinkControl);
                        ...
                        if (refreshMode === "onInterval") {
                            ...
                            if (defined(expires)) {
                                try {
                                    const date =
                                        JulianDate.fromIso8601(expires);
                ...
                .catch(function (error) {
                    oneTimeWarning(
                        `An error occured during loading ${href.url}`,
                    );
            ...
            if (/\.kml$/i.test(entry.filename)) {
                ...
                if (!defined(docEntry) || !/\//i.test(entry.filename)) {
                    if (defined(docEntry)) {
                        ...
                        promises.push(
                            loadDataUriFromZip(docEntry, uriResolver),
                        );
                    }
                    docEntry = entry;
                } else {
        ...
        .then(function () {
            ...
            if (!isMinStart || !isMaxStop) {
                ...
                clock.multiplier = Math.round(
                    Math.min(
                        Math.max(
                            JulianDate.secondsDifference(stop, start) / 60,
                            1,
                        ),
                        3.15569e7,
                    ),
