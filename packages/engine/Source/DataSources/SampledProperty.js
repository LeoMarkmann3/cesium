...
function mergeNewSamples(epoch, times, values, newData, packedLength) {
    ...
    while (newDataIndex < newData.length) {
        ...
        timesInsertionPoint = binarySearch(
            times,
            currentTime,
            JulianDate.compare,
        );
        ...
        if (timesInsertionPoint < 0) {
            ...
            while (newDataIndex < newData.length) {
                ...
                if (
                    ...
                    (defined(nextTime) &&
                        JulianDate.compare(currentTime, nextTime) >= 0)
                ) {
                    break;
                }
                ...
                for (i = 0; i < packedLength; i++) {
                    valuesSpliceArgs[valuesSpliceArgsCount++] =
            ...
            for (i = 0; i < packedLength; i++) {
                newDataIndex++;
                values[timesInsertionPoint * packedLength + i] =
                    newData[newDataIndex];
            }
            newDataIndex++;
        }
