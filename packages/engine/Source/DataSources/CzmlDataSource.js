...
function unwrapInterval(type, czmlInterval, sourceUri) {
    ...
    switch (type) {
        ...
        case HeightReference:
            return HeightReference[
                czmlInterval.heightReference ?? czmlInterval
            ];
        case HorizontalOrigin:
            return HorizontalOrigin[
                czmlInterval.horizontalOrigin ?? czmlInterval
            ];
        case Image:
        ...
        case StripeOrientation:
            return StripeOrientation[
                czmlInterval.stripeOrientation ?? czmlInterval
            ];
        case Rectangle:
        ...
        default:
            throw new RuntimeError(
                `Unknown CzmlDataSource interval type: ${type}`,
            );
    }
}
...
function updateInterpolationSettings(packetData, property) {
    ...
    const forwardExtrapolationDuration =
        packetData.forwardExtrapolationDuration;
...
function processProperty(
    ...
    const isValue =
        !defined(packetData.reference) &&
        !defined(packetData.velocityReference);
    const hasInterval =
    ...
    const needsUnpacking =
        typeof type.unpack === "function" && type !== Rotation;
    ...
    if (!isSampled && hasInterval) {
        ...
        if (isValue && property instanceof TimeIntervalCollectionProperty) {
            ...
            if (isValue) {
                combinedInterval.data = new ConstantProperty(
            ...
            object[propertyName] = property =
                convertPropertyToComposite(property);
...
function processPositionProperty(
    ...
    if (packetData.delete === true) {
        ...
        // Deleting depends on the type of property we have.
        return removePositionPropertyData(
            object[propertyName],
            combinedInterval,
        );
    }
    ...
    if (!isSampled && hasInterval) {
        ...
        if (!defined(property)) {
            if (isValue) {
                property = new TimeIntervalCollectionPositionProperty(
                    referenceFrame,
                );
            } else {
    ...
    if (
        ...
        (defined(referenceFrame) &&
            interval.data.referenceFrame !== referenceFrame)
    ) {
    ...
    if (defined(interval)) {
        ...
        if (!(property instanceof CompositePropertyArrayType)) {
            ...
            composite.intervals.addInterval(
                wrapPropertyInInfiniteInterval(property),
            );
...
PolygonHierarchyProperty.prototype.equals = function (other) {
    return (
        ...
        (other instanceof PolygonHierarchyProperty &&
            Property.equals(
                this.polygon._positions,
                other.polygon._positions,
            ) &&
...
function updateClock(dataSource) {
    ...
    if (!defined(clockPacket)) {
        if (!defined(dataSource._clock)) {
            const availability =
                dataSource._entityCollection.computeAvailability();
    ...
    if (defined(clockPacket.range)) {
        clock.clockRange =
            ClockRange[clockPacket.range] ?? ClockRange.LOOP_STOP;
    }
