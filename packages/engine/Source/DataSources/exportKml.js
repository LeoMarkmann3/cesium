...
exportKml._createState = function (options) {
    ...
    const time = defined(options.time)
        ? options.time
        : entityAvailability.start;
    ...
    for (let i = 0; i < count; ++i) {
        ...
        if (geometryCount > 0) {
            ...
            placemark.appendChild(
                createBasicElementWithText(kmlDoc, "name", name),
            );
            placemark.appendChild(
                createBasicElementWithText(kmlDoc, "visibility", entity.show),
            );
...
function createTracks(state, entity, pointGraphics, geometries, styles) {
    ...
    for (i = 0; i < intervals.length; ++i) {
        ...
        for (let k = 0; k < positionTimes.length; ++k) {
            const when = createBasicElementWithText(
                kmlDoc,
                "when",
                positionTimes[k],
            );
        ...
        if (isModel) {
            trackGeometry.appendChild(
                createModelGeometry(state, pointGraphics),
            );
        }
...
function createIconStyleFromBillboard(state, billboardGraphics) {
    ...
    if (defined(image)) {
        ...
        const imageSubRegion = valueGetter.get(
    ...
    if (defined(pixelOffset)) {
        ...
        const height = valueGetter.get(
            billboardGraphics.height,
            BILLBOARD_SIZE,
        );
    ...
    if (
        defined(rotation) &&
        Cartesian3.equals(Cartesian3.UNIT_Z, alignedAxis)
    ) {
...
function createLineString(state, polylineGraphics, geometries, styles) {
    ...
    // Set altitude mode
    const altitudeMode = kmlDoc.createElement("altitudeMode");
    const clampToGround = valueGetter.get(
...
function getPolygonBoundaries(state, polygonGraphics, extrudedHeight) {
    ...
    const positions = Array.isArray(hierarchy)
        ? hierarchy
        : hierarchy.positions;
...
function createModelGeometry(state, modelGraphics) {
    ...
    if (defined(scale)) {
        ...
        scaleElement.appendChild(
            createBasicElementWithText(kmlDoc, "x", scale),
        );
        scaleElement.appendChild(
            createBasicElementWithText(kmlDoc, "y", scale),
        );
        scaleElement.appendChild(
            createBasicElementWithText(kmlDoc, "z", scale),
        );
