...
function subSampleSampledProperty(
    ...
    while (t < len) {
        if (
            !steppedOnNow &&
            JulianDate.greaterThanOrEquals(current, updateTime)
        ) {
        ...
        if (t < len - 1) {
            if (maximumStep > 0 && !sampling) {
                ...
                const secondsUntilNext = JulianDate.secondsDifference(
                    next,
                    current,
                );
                ...
                if (sampling) {
                    sampleStepsToTake = Math.ceil(
                        secondsUntilNext / maximumStep,
                    );
                    sampleStepsTaken = 0;
                    sampleStepSize =
                        secondsUntilNext / Math.max(sampleStepsToTake, 2);
...
PolylineUpdater.prototype.updateObject = function (time, item) {
    ...
    if (show) {
        const leadTime = Property.getValueOrUndefined(
            pathGraphics._leadTime,
            time,
        );
        ...
        if (show) {
            if (hasTrailTime) {
                sampleStart = JulianDate.addSeconds(
                    time,
                    -trailTime,
                    new JulianDate(),
                );
            ...
            if (hasLeadTime) {
                sampleStop = JulianDate.addSeconds(
                    time,
                    leadTime,
                    new JulianDate(),
                );
            ...
            if (hasAvailability) {
                ...
                if (
                    !hasTrailTime ||
                    JulianDate.greaterThan(start, sampleStart)
                ) {
                    sampleStart = start;
                }
