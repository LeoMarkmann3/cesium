...
function orthometricToEllipsoidal(
    ...
    for (let i = 0; i < vertexCount; ++i) {
        const height = sampleGeoidFromList(
            ...
            center.latitude +
                CesiumMath.toRadians(scale_y * position[i * 3 + 1]),
            geoidDataList,
        );
...
function transformToLocal(
    ...
    for (let i = 0; i < vertexCount; ++i) {
        ...
        cartographic.height =
            cartographicCenter.height + positions[indexOffset2];
...
function cropUVs(vertexCount, uv0s, uvRegions) {
    for (let vertexIndex = 0; vertexIndex < vertexCount; ++vertexIndex) {
        ...
        const scaleU =
            (uvRegions[vertexIndex * 4 + 2] - uvRegions[vertexIndex * 4]) /
            65535.0;
        const scaleV =
    ...
    if (splitGeometryByColorTransparency && defined(colors)) {
        ...
        for (let vertexIndex = 0; vertexIndex < vertexCount; vertexIndex += 3) {
            if (
                ...
                indexArray[transparentVertexOffset++] =
                    vertexIndexFn(vertexIndex);
                indexArray[transparentVertexOffset++] = vertexIndexFn(
                    vertexIndex + 1,
                );
                indexArray[transparentVertexOffset++] = vertexIndexFn(
                    vertexIndex + 2,
...
function getFeatureHash(symbologyData, outlinesHash, featureIndex) {
    ...
    const featureSymbology =
        symbologyData[featureIndex] ?? symbologyData.default;
...
function convertColorsArray(colors) {
    ...
    for (let index = 0; index < colors.length; index += 4) {
        ...
        colorsArray[index + 1] = srgbToLinear(
            Color.byteToFloat(colors[index + 1]),
        );
        colorsArray[index + 2] = srgbToLinear(
            Color.byteToFloat(colors[index + 2]),
        );
        ...
        for (let attrIndex = 0; attrIndex < attributesCount; ++attrIndex) {
            ...
            if (metadata.ptr !== 0) {
                ...
                for (let entry = 0; entry < numEntries; ++entry) {
                    const entryName = metadataQuerier.GetEntryName(
                        metadata,
                        entry,
                    );
                    if (entryName === "i3s-scale_x") {
                        decodedGeometry.scale_x =
                            metadataQuerier.GetDoubleEntry(
                                metadata,
                                "i3s-scale_x",
                            );
                    } else if (entryName === "i3s-scale_y") {
                        decodedGeometry.scale_y =
                            metadataQuerier.GetDoubleEntry(
                                metadata,
                                "i3s-scale_y",
                            );
...
function decodeDracoAttribute(
    ...
    const handlers = [
        ...
        function () {
            // DT_INT8 - 1
            dracoAttributeData = new dracoDecoderModule.DracoInt8Array(
                bufferSize,
            );
        ...
        function () {
            // DT_INT16 - 3
            dracoAttributeData = new dracoDecoderModule.DracoInt16Array(
                bufferSize,
            );
        ...
        function () {
            // DT_INT32 - 5
            dracoAttributeData = new dracoDecoderModule.DracoInt32Array(
                bufferSize,
            );
        ...
        function () {
            // DT_FLOAT32 - 11
            dracoAttributeData = new dracoDecoderModule.DracoUInt8Array(
                bufferSize,
            );
            const success = dracoDecoder.GetAttributeUInt8ForAllPoints(
                dracoGeometry,
        ...
        if (defined(bufferInfo)) {
            for (
                ...
                if (
                    ...
                    offset = binaryAttributeDecoders[
                        bufferInfo.attributes[attrIndex]
                    ](decodedGeometry, data, offset);
                } else {
                    console.error(
                        "Unknown decoder for",
                        bufferInfo.attributes[attrIndex],
            ...
            for (let j = 0; j < featureAttributeOrder.length; j++) {
                const curDecoder =
                    binaryAttributeDecoders[featureAttributeOrder[j]];
