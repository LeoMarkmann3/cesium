...
function processBuffer(
    ...
    for (quad = 0; quad < 4; ++quad) {
        ...
        const originX = CesiumMath.toRadians(
            dv.getFloat64(offset, true) * 180.0,
        );
        offset += sizeOfDouble;

        const originY = CesiumMath.toRadians(
            dv.getFloat64(offset, true) * 180.0,
        );
        offset += sizeOfDouble;
        ...
        for (let i = 0; i < numPoints; ++i) {
            ...
            if (
                ...
                if (index === -1) {
                    quadBorderPoints.push(
                        Cartographic.clone(scratchCartographic),
                    );
            ...
            let v =
                (latitude - geographicSouth) /
                (geographicNorth - geographicSouth);
