...
function createResources(pointCloud, frameState) {
    ...
    if (hasStyleableProperties) {
        ...
        for (const name in styleableProperties) {
            if (styleableProperties.hasOwnProperty(name)) {
                ...
                const typedArray = prepareVertexAttribute(
                    property.typedArray,
                    name,
                );
    ...
    if (pointCloud._cull) {
        if (isQuantized || isQuantizedDraco) {
            ...
            pointCloud._boundingSphere =
                computeApproximateBoundingSphereFromPositions(
    ...
    if (pointCloud._opaquePass === Pass.CESIUM_3D_TILE) {
        ...
        translucentRenderState.stencilTest =
            StencilConstants.setCesium3DTileBit();
        translucentRenderState.stencilMask =
    ...
    let uniformMap = {
        u_pointSizeAndTimeAndGeometricErrorAndDepthMultiplier: function () {
            ...
            if (pointCloud._attenuation) {
                ...
                scratch.z =
                    pointCloud.geometricError * pointCloud.geometricErrorScale;
...
function createShaders(pointCloud, frameState, style) {
    ...
    for (name in styleableShaderAttributes) {
        if (styleableShaderAttributes.hasOwnProperty(name)) {
            attribute = styleableShaderAttributes[name];
            const enabled =
                styleablePropertyIds.indexOf(attribute.location) >= 0;
    ...
    if (hasColors) {
        ...
        const colorVertexAttribute = getVertexAttribute(
            vertexArray,
            colorLocation,
        );
            ...
            decodePromise
                .then(function (result) {
                    ...
                    const decodedPositions = defined(result.POSITION)
                        ...
                        : undefined;
                    const decodedRgb = defined(result.RGB)
                        ? result.RGB.array
                        : undefined;
                    ...
                    const isOctEncodedDraco =
                        defined(decodedNormals) &&
                        defined(result.NORMAL.data.quantization);
                    if (isQuantizedDraco) {
                        ...
                        pointCloud._quantizedVolumeScale =
                            Cartesian3.fromElements(range, range, range);
                    ...
                    if (isOctEncodedDraco) {
                        pointCloud._octEncodedRange =
                            (1 <<
                                result.NORMAL.data.quantization
                                    .quantizationBits) -
                            1.0;
                    ...
                    for (const name in batchTableProperties) {
                        if (batchTableProperties.hasOwnProperty(name)) {
                            ...
                            styleableProperties[name] = {
                                ...
                                componentCount:
                                    property.data.componentsPerAttribute,
                            };
                        }
                    }
