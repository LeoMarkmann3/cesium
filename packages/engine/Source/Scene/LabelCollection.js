...
function rebindAllGlyphs(labelCollection, label) {
    ...
    if (textLength < glyphsLength) {
        for (
            let glyphIndex = textLength;
            glyphIndex < glyphsLength;
            ++glyphIndex
        ) {
    ...
    for (let textIndex = 0; textIndex < textLength; ++textIndex) {
        ...
        if (!defined(glyphBillboardTexture) || !defined(dimensions)) {
            glyphBillboardTexture = new BillboardTexture(
                glyphBillboardCollection,
            );
...
function repositionAllGlyphs(label) {
    ...
    const lineSpacing =
        (defined(label._lineHeight)
            ? label._lineHeight
            : defaultLineSpacingPercent * label._fontSize) /
        label._relativeSize;
    ...
    if (defined(backgroundBillboard) && text.split("\n").join("").length > 0) {
        ...
        if (verticalOrigin === VerticalOrigin.TOP) {
            ...
            glyphPixelOffset.y =
                (maxLineHeight - maxGlyphY) / 2 - maxGlyphDescent;
        } else if (verticalOrigin === VerticalOrigin.BASELINE) {
    ...
    ready: {
        get: function () {
            const backgroundBillboard =
                this._backgroundBillboardCollection.get(0);
