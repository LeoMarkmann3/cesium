            ...
            for (let ii = 0; ii < length; ++ii) {
                ...
                if (this._batchTable.attributes.length > 2) {
                    if (
                        ...
                        properties[POSITION_SIZE_INDEX]
                    ) {
...
function createVertexArrays(collection, context, projection) {
    ...
    if (totalLength > 0) {
        ...
        const texCoordExpandAndBatchIndexArray = new Float32Array(
            totalLength * 4,
        );
        let position3DArray;
        ...
        collection._texCoordExpandAndBatchIndexBuffer =
        ...
        for (let k = 0; k < numberOfIndicesArrays; ++k) {
            ...
            if (indices.length > 0) {
                ...
                const positionLowOffset =
                    positionSizeInBytes + positionHighOffset;
                const prevPositionHighOffset =
                    positionSizeInBytes + positionLowOffset;
...
function intersectsIDL(polyline) {
    return (
        Cartesian3.dot(Cartesian3.UNIT_X, polyline._boundingVolume.center) <
            0 ||
    ...
    for (let i = 0; i < length; ++i) {
        ...
        for (let j = 0; j < positionsLength; ++j) {
            ...
            for (let k = startK; k < endK; ++k) {
                ...
                texCoordExpandAndBatchIndexArray[
                    texCoordExpandAndBatchIndexIndex + 1
            ...
            for (let j = 0; j < numberOfSegments; ++j) {
                ...
                for (let k = 0; k < segmentLength; ++k) {
                    if (indicesCount + 4 > CesiumMath.SIXTY_FOUR_KILOBYTES) {
                        ...
                        count = 0;
                        offset = 0;
                        bucketLocator = new VertexArrayBucketLocator(
                            0,
                            0,
                            this,
                        );
