...
function preprocess(layers) {
    ...
    for (i = 0; i < layersLength; i++) {
        ...
        for (j = 0; j < entriesLength - 1; j++) {
            ...
            sortedAscending =
                sortedAscending && currEntry.height <= nextEntry.height;
            sortedDescending =
...
function createLayeredEntries(layers) {
    ...
    function addBlendEntry(height, a, b) {
        let result = Color.multiplyByScalar(
            b,
            1.0 - a.alpha,
            scratchColorBlend,
        );
    ...
    for (i = 0; i < layerLength; i++) {
        ...
        while (idx < entriesLength || accumIdx < entriesAccumLength) {
            ...
            const entryAccum =
                accumIdx < entriesAccumLength
                    ? entriesAccumCurr[accumIdx]
                    : undefined;
            ...
            if (
                ...
                const isSplit =
                    defined(nextEntry) && entry.height === nextEntry.height;
...
function createElevationBandMaterial(options) {
    ...
    const isPackedHeight =
        !createElevationBandMaterial._useFloatTexture(context);
    if (isPackedHeight) {
        ...
        heightTexFormat = context.webgl2
            ? PixelFormat.RED
