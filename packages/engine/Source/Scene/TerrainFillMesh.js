...
function createFillMesh(tileProvider, frameState, tile, vertexArraysToDestroy) {
    ...
    if (
        ...
        if (availableBytesInBuffer >= indexDataBytes) {
            ...
            const startIndex =
                vertexCount * stride * Float32Array.BYTES_PER_ELEMENT;
            indices =
                vertexCount < 256
    ...
    if (tileProvider.terrainProvider.hasWaterMask) {
        const waterSourceTile =
            surfaceTile._findAncestorTileWithTerrainData(tile);
        if (
...
function transformTextureCoordinates(
    ...
    if (
        ...
        coordinates.x === 1.0 &&
        sourceTile.x ===
            sourceTile.tilingScheme.getNumberOfXTilesAtLevel(sourceTile.level) -
                1
    ) {
        ...
        coordinates.x === 0.0 &&
        targetTile.x ===
            targetTile.tilingScheme.getNumberOfXTilesAtLevel(targetTile.level) -
                1
    ) {
        sourceRectangle = Rectangle.clone(
            sourceTile.rectangle,
    ...
    if (meshIsUsable(sourceTile, sourceMesh)) {
        ...
        if (edgeVertices.length > 0) {
            ...
            if (vertexIndexIndex < 0) {
                ...
                if (
                    vertexIndexIndex > 0 &&
                    vertexIndexIndex < edgeVertices.length
                ) {
