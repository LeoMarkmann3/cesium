...
PrimitivePipeline.combineGeometry = function (parameters) {
    ...
    for (let i = 0; i < length; ++i) {
        ...
        if (
            defined(eastHemisphereGeometry) &&
            defined(westHemisphereGeometry)
        ) {
            if (
...
PrimitivePipeline.packCreateGeometryResults = function (
    ...
    for (let i = 0; i < length; i++) {
        ...
        const validBoundingSphere = defined(geometry.boundingSphere)
            ? 1.0
            : 0.0;
...
function packInstancesForCombine(instances, transferableObjects) {
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (
            ...
            defined(instance.attributes.offset)
        ) {
...
PrimitivePipeline.unpackCombineGeometryResults = function (packedResult) {
    return {
        ...
        boundingSpheresCV: unpackBoundingSpheres(
            packedResult.boundingSpheresCV,
        ),
    };
};
