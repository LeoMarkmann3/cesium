...
function maintainInertia(
    ...
    if (ts && tr && threshold < inertiaMaxClickTimeThreshold) {
        ...
        if (!aggregator.isButtonDown(type, modifier)) {
            const startPosition = aggregator.getStartMousePosition(
                type,
                modifier,
            );
...
function activateInertia(controller, inertiaStateName) {
    if (defined(inertiaStateName)) {
        ...
        const inertiasToDisable =
            controller._inertiaDisablers[inertiaStateName];
    ...
    if (!sameStartPosition || rotatingZoom) {
        if (mode === SceneMode.SCENE2D) {
            ...
            if (
                ...
                const d =
                    (Cartesian3.distance(worldPosition, endPosition) *
                        distance) /
            ...
            if (
                ...
                (camera.positionCartographic.height < 3000.0 &&
                    Math.abs(
                        Cartesian3.dot(camera.direction, cameraPositionNormal),
                    ) < 0.6)
            ) {
                zoomOnVector = true;
            } else {
                ...
                if (!defined(centerPosition)) {
                    ...
                    if (
                        ...
                        targetNormal = Cartesian3.normalize(
                            target,
                            targetNormal,
                        );

                        if (
                            Cartesian3.dot(targetNormal, cameraPositionNormal) <
                            0.0
                        ) {
                            return;
                        }
                        ...
                        const positionToTargetNormal =
                            scratchPositionToTargetNormal;
                        Cartesian3.subtract(
                            target,
                            cameraPosition,
                            positionToTarget,
                        ...
                        const gamma = Math.asin(
                            CesiumMath.clamp(
                                (positionToTargetDistance / targetDistance) *
                                    Math.sin(alpha),
                                -1.0,
                                1.0,
                            ),
...
function translateCV(controller, startPosition, movement) {
    ...
    const endMouse = Cartesian2.clone(
        movement.endPosition,
        translateCVEndMouse,
    );
    ...
    if (Cartesian2.equals(startPosition, controller._tiltCenterMousePosition)) {
        ...
        if (!defined(center)) {
            ...
            if (!defined(scalar) || scalar <= 0.0) {
                ...
                Cartesian2.clone(
                    startPosition,
                    controller._tiltCenterMousePosition,
                );
                return;
            }
...
function zoomCV(controller, startPosition, movement) {
    ...
    if (!defined(distance)) {
        const normal = Cartesian3.UNIT_X;
        distance =
            -Cartesian3.dot(normal, position) /
            Cartesian3.dot(normal, direction);
    }
    ...
    if (defined(globe) && height < controller._minimumPickingTerrainHeight) {
        ...
        if (defined(mousePos)) {
            ...
            if (strafing) {
                Cartesian2.clone(
                    startPosition,
                    controller._strafeEndMousePosition,
                );
...
function rotate3D(
    ...
    let rotateRate =
        controller._rotateFactor *
        (rho - controller._rotateRateRangeAdjustment);
...
function pan3D(controller, startPosition, movement, ellipsoid) {
    ...
    if (
        ...
        if (!defined(controller._globe) && defined(p0)) {
            ...
            const magnitude =
                ((-dragDelta.y * pixelDimensions.y * 2.0) /
                    Math.sqrt(forward)) *
                (1.0 - dot);
    ...
    if (!defined(camera.constrainedAxis)) {
        ...
        const startRej = Cartesian3.multiplyByScalar(
            basis0,
            startDot,
            pan3DTemp2,
        );
...
function look3D(controller, startPosition, movement, rotationAxis) {
    ...
    if (defined(rotationAxis)) {
        ...
        if (!northParallel && !southParallel) {
            ...
            (northParallel && angle < 0) ||
            (southParallel && angle > 0)
        ) {
...
function adjustHeightForTerrain(controller, cameraChanged) {
    ...
    let mag;
    if (!Matrix4.equals(camera.transform, Matrix4.IDENTITY)) {
        transform = Matrix4.clone(
            camera.transform,
            scratchAdjustHeightTransform,
        );
