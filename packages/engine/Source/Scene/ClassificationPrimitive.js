...
function createRenderStates(
    ...
    classificationPrimitive._rsPickPass =
        RenderState.fromCache(pickRenderState);
}
...
function createShaderProgram(classificationPrimitive, frameState) {
    ...
    if (classificationPrimitive._primitive.allowPicking) {
        ...
        if (cullFragmentsUsingExtents) {
            ...
            if (!defined(pickProgram2D)) {
                const pickFS2D =
                    shadowVolumeAppearance.createPickFragmentShader(true);
    ...
    if (cullFragmentsUsingExtents) {
        ...
        if (!defined(colorProgram2D)) {
            const fsColorSource2D =
                shadowVolumeAppearance.createFragmentShader(true);
...
function createColorCommands(classificationPrimitive, colorCommands) {
    ...
    for (let j = 0; j < length; ++j) {
        const commandIgnoreShow = (commandsIgnoreShow[j] =
            DrawCommand.shallowClone(
...
function updateAndQueueCommands(
    ...
    if (passes.render) {
        ...
        for (i = 0; i < colorLength; ++i) {
            boundingVolume =
                boundingVolumes[boundingVolumeIndex(i, colorLength)];
        ...
        if (frameState.invertClassification) {
            const ignoreShowCommands =
                classificationPrimitive._commandsIgnoreShow;
