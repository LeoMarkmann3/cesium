...
function createVertexArray(polylines, context) {
    ...
    if (!defined(packedBuffer)) {
        ...
        batchIds = polylines._transferrableBatchIds =
            polylines._batchIds.slice();
...
function finishVertexArray(polylines, context) {
    if (!defined(polylines._va)) {
        ...
        const endFaceNormalAndHalfWidths =
            polylines._endFaceNormalAndHalfWidths;
        ...
        byteLength +=
            startPositionAndHeights.byteLength +
            endPositionAndHeights.byteLength;
        byteLength +=
        ...
        const startFaceNormalAndVertexCornerIdsBuffer =
...
Vector3DTileClampedPolylines.prototype.update = function (frameState) {
    ...
    if (!this._ready) {
        if (!defined(this._promise)) {
            this._promise = initialize(this).then(
