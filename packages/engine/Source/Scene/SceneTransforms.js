...
SceneTransforms.worldToDrawingBufferCoordinates = function (
    ...
    return SceneTransforms.transformWindowToDrawingBuffer(
        scene,
        result,
        result,
    );
...
SceneTransforms.drawingBufferToWorldCoordinates = function (
    ...
    ndc.x =
        ((drawingBufferPosition.x - viewport.x) / viewport.width) * 2.0 - 1.0;
    ndc.y =
    ...
    if (!defined(frustum.fovy)) {
        ...
        worldCoords.x =
            (ndc.x * (frustum.right - frustum.left) +
                frustum.left +
                frustum.right) *
            0.5;
        worldCoords.y =
            (ndc.y * (frustum.top - frustum.bottom) +
                frustum.bottom +
                frustum.top) *
            0.5;
