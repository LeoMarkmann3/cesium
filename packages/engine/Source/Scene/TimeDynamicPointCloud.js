...
function handleFrameFailure(that, uri) {
    return function (error) {
        const message = defined(error.message)
            ? error.message
            : error.toString();
    ...
    for (let i = 0; i < length; ++i) {
        ...
        if (defined(frame)) {
            if (!defined(unloadCondition) || unloadCondition(frame)) {
                const pointCloud = frame.pointCloud;
                if (frame.ready) {
                    that._totalMemoryUsageInBytes -=
                        pointCloud.geometryByteLength;
                }
...
function getNearestReadyInterval(
    ...
    if (currentIndex >= previousIndex) {
        ...
        for (i = currentIndex; i >= previousIndex; --i) {
            ...
            frame = frames[i];
            if (
                updateInterval(that, interval, frame, updateState, frameState)
            ) {
                return interval;
            }
        }
...
TimeDynamicPointCloud.prototype.update = function (frameState) {
    ...
    const timeSinceLoad = Math.max(
        JulianDate.secondsDifference(frameState.time, this._loadTimestamp) *
            1000,
        0.0,
    );
