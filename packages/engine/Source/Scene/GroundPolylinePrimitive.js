...
function createShaderProgram(groundPolylinePrimitive, frameState, appearance) {
    ...
    if (defined(appearance.material)) {
        ...
        if (
            materialShaderSource.search(/in\s+float\s+v_polylineAngle;/g) !== -1
        ) {
...
function createCommands(
    ...
    const materialUniforms = isPolylineColorAppearance
        ? {}
        : material._uniforms;
    const uniformMap =
    ...
    for (let i = 0; i < length; i++) {
        ...
        command.pickId =
            "czm_batchTable_pickColor(v_endPlaneNormalEcAndBatchId.w)";
        ...
        derivedMorphCommand.renderState =
            groundPolylinePrimitive._renderStateMorph;
