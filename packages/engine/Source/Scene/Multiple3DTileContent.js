    ...
    metadata: {
        ...
        set: function () {
            ...
            throw new DeveloperError(
                "Multiple3DTileContent cannot have metadata",
            );
            //>>includeEnd('debug');
        },
    },
...
function canScheduleAllRequests(serverKeys) {
    ...
    for (const key in requestCountsByServer) {
        if (
            ...
            !RequestScheduler.serverHasOpenSlots(
                key,
                requestCountsByServer[key],
            )
        ) {
            return false;
        }
...
async function createInnerContents(multipleContents) {
    ...
    const arrayBuffers = await Promise.all(
        multipleContents._arrayFetchPromises,
    );
    ...
    if (
        ...
        multipleContents._contents.length
    ) {
...
async function createInnerContent(multipleContents, arrayBuffer, index) {
    ...
    try {
        ...
        if (
            ...
            Cesium3DTileContentType.EXTERNAL_TILESET
        ) {
            multipleContents._externalTilesetCount++;
        ...
        if (tile.hasImplicitContentMetadata) {
            ...
            content.metadata = subtree.getContentMetadataView(
                coordinates,
                index,
            );
