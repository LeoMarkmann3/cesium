...
function updateKeyframeNodes(that, frameState) {
    ...
    for (
        ...
        if (
            highPriorityKeyframeNode.state === KeyframeNode.LoadState.UNLOADED
        ) {
            requestData(that, highPriorityKeyframeNode);
        }
        if (
            highPriorityKeyframeNode.state === KeyframeNode.LoadState.PROCESSING
        ) {
            ...
            if (megatexture.isFull()) {
                ...
                addNodeIndex =
                    keyframeNodesInMegatextureCount - 1 - destroyedCount;
                destroyedCount++;
    ...
    function buildOctree(
        ...
        if (defined(node.children)) {
            for (let c = 0; c < 8; c++) {
                ...
                childNode.computeSurroundingRenderableKeyframeNodes(
                    keyframeLocation,
                );
        ...
        if (hasRenderableChildren) {
            ...
            if (useLeafNodes) {
                ...
                const levelDifference =
                    node.level - keyframeNode.spatialNode.level;
                ...
                const megatextureIndexParent =
                    parentKeyframeNode.megatextureIndex;
