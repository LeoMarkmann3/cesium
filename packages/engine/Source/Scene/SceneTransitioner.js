...
function morphFrom3DTo2D(transitioner, duration, ellipsoid) {
    ...
    if (duration > 0.0) {
        ...
        const rayDirection = Cartesian3.clone(
            camera.directionWC,
            ray.direction,
        );
        const surfacePoint = ellipsoid.scaleToGeodeticSurface(
...
function morphFrom2DToColumbusView(transitioner, duration, cameraCV, complete) {
    ...
    function morph() {
        ...
        const startPos = Cartesian3.clone(
            camera.position,
            scratch3DToCVStartPos,
        );
        const startDir = Cartesian3.clone(
            camera.direction,
            scratch3DToCVStartDir,
        );
