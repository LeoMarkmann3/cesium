...
function selectTilesForRendering(primitive, frameState) {
    ...
    primitive._occluders.ellipsoid.cameraPosition =
...
function visitTile(
    ...
    if (meetsSse || ancestorMeetsSse) {
        ...
        let renderable =
            oneRenderedLastFrame ||
            twoCulledOrNotVisited ||
            threeCompletelyLoaded;
    ...
    if (tileProvider.canRefine(tile)) {
        ...
        if (firstRenderedDescendantIndex !== primitive._tilesToRender.length) {
            ...
            if (!allAreRenderable && !anyWereRenderedLastFrame) {
                ...
                for (
                    let i = firstRenderedDescendantIndex;
                    i < renderList.length;
                    ++i
                ) {
                    ...
                    while (
                        ...
                        workTile._lastSelectionResult !==
                            TileSelectionResult.KICKED &&
                        workTile !== tile
                    ) {
                        workTile._lastSelectionResult =
                            TileSelectionResult.kick(
                                workTile._lastSelectionResult,
                            );
                ...
                primitive._tileToUpdateHeights.length =
                ...
                if (
                    ...
                    traversalDetails.notYetRenderableCount = tile.renderable
                        ? 0
                        : 1;
                    queuedForLoad = true;
                }
...
function containsNeededPosition(primitive, tile) {
    ...
    return (
        (defined(primitive._cameraPositionCartographic) &&
            Rectangle.contains(
                rectangle,
                primitive._cameraPositionCartographic,
            )) ||
...
function screenSpaceError(primitive, frameState, tile) {
    ...
    if (frameState.fog.enabled) {
        error -=
            CesiumMath.fog(distance, frameState.fog.density) *
            frameState.fog.sse;
    }
    ...
    while (tilesToUpdateHeights.length > 0) {
        ...
        if (!defined(tile.data) || !defined(tile.data.mesh)) {
            ...
            const selectionResult =
                ...
                primitive._lastSelectionFrameNumber
                    ? tile._lastSelectionResult
                    : TileSelectionResult.NONE;
            if (
            ...
            if (tile.level > data.level && !upsampledGeometryFromParent) {
                ...
                if (defined(cachedData)) {
                    ...
                    if (mode === SceneMode.SCENE3D) {
                        ...
                        const rayOrigin =
                            ellipsoid.getSurfaceNormalIntersectionWithZAxis(
                                data.positionOnEllipsoidSurface,
                                11500.0,
                                scratchRay.origin,
                            );
                        ...
                        if (!defined(rayOrigin)) {
                            ...
                            if (defined(tile.data.tileBoundingRegion)) {
                                minimumHeight =
                                    tile.data.tileBoundingRegion.minimumHeight;
                            }
                            const magnitude = Math.min(minimumHeight, -11500.0);

                            // multiply by the *positive* value of the magnitude
                            const vectorToMinimumPoint =
