...
class ModelReader {
    ...
    static readAttributeAsRawCompactTypedArray(attribute) {
        ...
        const componentsReader =
            ModelReader.createComponentsReader(componentType);
    ...
    static octDecode(
        ...
        if (!defined(dequantizedTypedArray)) {
            dequantizedTypedArray = new Float32Array(
                quantizedTypedArray.length,
            );
        }
    ...
    static createComponentsReader(componentType) {
        const componentReader =
            ModelReader.createComponentReader(componentType);
    ...
    static readIndicesAsTriangleIndicesTypedArray(
        ...
        if (primitiveType === PrimitiveType.TRIANGLE_STRIP) {
            const triangleIndices =
                ModelReader.convertTriangleStripToTriangleIndices(
                    originalIndices,
                );
        ...
        if (primitiveType === PrimitiveType.TRIANGLE_FAN) {
            const triangleIndices =
                ModelReader.convertTriangleFanToTriangleIndices(
                    originalIndices,
                );
            return triangleIndices;
        }
