    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        if (defined(instances)) {
            ...
            if (!defined(transformsTypedArray)) {
                const instanceTransformsBuffer =
                    runtimeNode.instancingTransformsBuffer;
                if (
                    defined(instanceTransformsBuffer) &&
                    ...
                    instanceTransformsBuffer.getBufferData(
        ...
        for (let j = 0; j < primitivesLength; j++) {
            ...
            if (
                defined(runtimePrimitive.boundingSphere) &&
                !defined(instances)
            ) {
            ...
            if (defined(quantization)) {
                componentDatatype =
                    positionAttribute.quantization.componentDatatype;
                attributeType = positionAttribute.quantization.type;
            }

            const numComponents =
                AttributeType.getNumberOfComponents(attributeType);
