...
function processDracoAttributes(loader, draco, result) {
    ...
    if (defined(result.POSITION)) {
        ...
        if (defined(result.POSITION.data.quantization)) {
            ...
            const quantizedVolumeScale = Cartesian3.fromElements(
                range,
                range,
                range,
            );
            const quantizedVolumeOffset = Cartesian3.unpack(
...
function makeAttribute(loader, attributeInfo, context) {
    ...
    if (attributeInfo.octEncoded) {
        ...
        quantization.componentDatatype =
            attributeInfo.quantizedComponentDatatype;
    }
    ...
    if (defined(attributeInfo.constantColor)) {
        ...
        attribute.constant = Color.pack(
            attributeInfo.constantColor,
            packedColor,
        );
    } else {
