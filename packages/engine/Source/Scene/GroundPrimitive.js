...
Object.defineProperties(GroundPrimitive.prototype, {
    ...
    releaseGeometryInstances: {
        get: function () {
            return this._classificationPrimitiveOptions
                .releaseGeometryInstances;
        },
    },
...
function updateAndQueueCommands(
    ...
    if (passes.pick) {
        ...
        for (i = 0; i < pickLength; ++i) {
            boundingVolume =
                boundingVolumes[boundingVolumeIndex(i, pickLength)];
            if (!groundPrimitive._useFragmentCulling) {
                const pickOffset =
                    pickOffsets[boundingVolumeIndex(i, pickLength)];
        ...
        for (i = 0; i < length; ++i) {
            ...
            if (defined(id) && defined(instanceRectangle)) {
                const boundingSphere =
                    ApproximateTerrainHeights.getBoundingSphere(
                        instanceRectangle,
                        ellipsoid,
                    );
                ...
                for (const attributeKey in instanceAttributes) {
                    if (instanceAttributes.hasOwnProperty(attributeKey)) {
                        attributes[attributeKey] =
                            instanceAttributes[attributeKey];
                    }
                }
