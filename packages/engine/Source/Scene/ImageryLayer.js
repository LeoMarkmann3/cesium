...
ImageryLayer.prototype._createTileImagerySkeletons = function (
    ...
    const useWebMercatorT =
        imageryProvider.tilingScheme.projection instanceof
    ...
    if (
        Math.abs(northwestTileRectangle.east - tile.rectangle.west) <
            veryCloseX &&
    ...
    if (
        Math.abs(southeastTileRectangle.west - tile.rectangle.east) <
            veryCloseX &&
    ...
    if (useWebMercatorT) {
        ...
        imageryTileXYToRectangle =
            imageryTilingScheme.tileXYToNativeRectangle.bind(
                imageryTilingScheme,
            );
    ...
    if (
        !this.isBaseLayer() &&
        Math.abs(clippedImageryRectangle.west - terrainRectangle.west) >=
            veryCloseX
    ) {
        maxU = Math.min(
            1.0,
...
ImageryLayer.prototype._calculateTextureTranslationAndScale = function (
    ...
    return new Cartesian4(
        (scaleX * (terrainRectangle.west - imageryRectangle.west)) /
            terrainWidth,
...
ImageryLayer.prototype._finalizeReprojectTexture = function (context, texture) {
    ...
    if (
        ...
        if (!defined(nonMipmapSampler)) {
            nonMipmapSampler = nonMipmapSamplers[nonMipmapSamplerKey] =
                new Sampler({
                    wrapS: TextureWrap.CLAMP_TO_EDGE,
                    wrapT: TextureWrap.CLAMP_TO_EDGE,
                    minificationFilter: minificationFilter,
...
function reprojectToGeographic(command, context, texture, rectangle) {
    ...
    const southMercatorY =
        0.5 * Math.log((1 + sinLatitude) / (1 - sinLatitude));
