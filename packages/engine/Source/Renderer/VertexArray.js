...
function addAttribute(attributes, attribute, index, context) {
    ...
    if (!hasVertexBuffer && !hasValue) {
        throw new DeveloperError(
            "attribute must have a vertexBuffer or a value.",
        );
    }
    if (hasVertexBuffer && hasValue) {
        throw new DeveloperError(
...
function VertexArray(options) {
    ...
    for (i = 0; i < length; ++i) {
        ...
        if (
            defined(attribute.vertexBuffer) &&
            attribute.instanceDivisor === 0
        ) {
...
function interleaveAttributes(attributes) {
    ...
    names.sort(function (left, right) {
        return (
            ComponentDatatype.getSizeInBytes(
                attributes[right].componentDatatype,
            ) -
        ...
        for (j = 0; j < numberOfVertices; ++j) {
            for (let n = 0; n < namesLength; ++n) {
                ...
                for (let k = 0; k < numberOfComponents; ++k) {
                    pointer[view.index + k] =
                        values[j * numberOfComponents + k];
                }
...
VertexArray.fromGeometry = function (options) {
    ...
    const attributeLocations =
        options.attributeLocations ?? Frozen.EMPTY_OBJECT;
    ...
    let attribute;
    let vertexBuffer;
    const vaAttributes = defined(createdVAAttributes)
        ? createdVAAttributes
        : [];
