...
function handleUniformPrecisionMismatches(
    ...
    if (
        !ContextLimits.highpFloatSupported ||
        !ContextLimits.highpIntSupported
    ) {
        let i, j;
        let uniformName;
        let duplicateName;
        ...
        for (i = 0; i < vertexUniformsCount; i++) {
            for (j = 0; j < fragmentUniformsCount; j++) {
                if (vertexShaderUniforms[i] === fragmentShaderUniforms[j]) {
                    ...
                    fragmentShaderText = fragmentShaderText.replace(
                        re,
                        duplicateName,
                    );
...
function createAndLinkProgram(gl, shader) {
    ...
    if (gl.getProgramParameter(program, gl.LINK_STATUS)) {
        if (shader._logShaderCompilation) {
            ...
            if (defined(log) && log.length > 0) {
                console.log(
                    `${consolePrefix}Vertex shader compile log: ${log}`,
                );
            }
            ...
            if (defined(log) && log.length > 0) {
                console.log(
                    `${consolePrefix}Fragment shader compile log: ${log}`,
                );
            }
    ...
    function logTranslatedSource(compiledShader, name) {
        ...
        const translation =
            debugShaders.getTranslatedShaderSource(compiledShader);
        if (translation === "") {
        ...
        if (uniformName.indexOf("gl_") !== 0) {
            if (activeUniform.name.indexOf("[") < 0) {
                ...
                if (indexOfBracket >= 0) {
                    ...
                    uniformArray =
                        uniformsByName[uniformName.slice(0, indexOfBracket)];
...
function reinitialize(shader) {
    ...
    if (typeof spector !== "undefined") {
        shader._program.__SPECTOR_rebuildProgram = function (
            ...
            shader._fragmentShaderText = fragmentSourceCode.replace(
                regex,
                " != ",
            );
