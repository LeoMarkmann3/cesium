...
RectangleGeometryLibrary.computePosition = function (
    ...
    if (computeST) {
        ...
        if (defined(stNwCorner)) {
            ...
            st.x =
                (stLongitude - computedOptions.stWest) *
                computedOptions.lonScalar;
            st.y =
                (stLatitude - computedOptions.stSouth) *
                computedOptions.latScalar;
        } else {
...
function getRotationOptions(
    ...
    nwCartesian = Cartesian3.subtract(
        nwCartesian,
        centerCartesian,
        nwCartesian,
    );
    const rotationMatrix = Matrix2.fromRotation(
        rotation,
...
RectangleGeometryLibrary.computeOptions = function (
    ...
    if (stRotation !== 0) {
        ...
        const stNwCorner = Rectangle.northwest(
            boundingRectangle,
            stNwCornerResult,
        );
