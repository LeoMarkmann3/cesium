...
function computeRoundCorner(
    ...
    if (granularity > 0) {
        ...
        for (let i = 0; i < granularity; i++) {
            ...
            surfacePoint = ellipsoid.scaleToGeodeticSurface(
                startPoint,
                scratch2,
            );
            finalPositions = addPosition(
...
PolylineVolumeGeometryLibrary.computePositions = function (
    ...
    for (let i = 1; i < length - 1; i++) {
        ...
        forward = Cartesian3.subtract(nextPosition, position, forward);
        forward = Cartesian3.normalize(forward, forward);
        surfaceNormal = ellipsoid.geodeticSurfaceNormal(
            position,
            surfaceNormal,
        );
        ...
        if (doCorner) {
            cornerDirection = Cartesian3.add(
                forward,
                backward,
                cornerDirection,
            );
            ...
            if (leftIsOutside) {
                ...
                scratch2Array[0] = Cartesian3.clone(
                    previousPosition,
                    scratch2Array[0],
                );
                ...
                if (
                    ...
                    cornerDirection = Cartesian3.negate(
                        cornerDirection,
                        cornerDirection,
                    );
                    finalPositions = addPosition(
