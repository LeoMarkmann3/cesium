...
EarthOrientationParameters.NONE = Object.freeze({
    compute: function (date, result) {
        if (!defined(result)) {
            result = new EarthOrientationParametersSample(
                0.0,
                0.0,
                0.0,
                0.0,
...
function fillResultFromIndex(eop, samples, index, columnCount, result) {
    ...
    result.xPoleOffset =
        samples[start + eop._xCelestialPoleOffsetRadiansColumn];
    result.yPoleOffset =
        samples[start + eop._yCelestialPoleOffsetRadiansColumn];
...
function interpolate(eop, dates, samples, date, before, after, result) {
    ...
    let beforeUt1MinusUtc =
        samples[startBefore + eop._ut1MinusUtcSecondsColumn];
