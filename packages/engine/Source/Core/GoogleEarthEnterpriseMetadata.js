            ...
            for (let i = 0; i < keysLength; ++i) {
                ...
                if (r !== null) {
                    ...
                    if (keyLength === topLevelKeyLength) {
                        ...
                        const parent =
                            tileInfo[key.substring(0, key.length - 1)];
...
function requestDbRoot(that) {
    ...
    return dbrootParserPromise
        ...
        .then(function (buf) {
            const encryptedDbRootProto =
                dbrootParser.EncryptedDbRootProto.decode(new Uint8Array(buf));
        ...
        .then(function (result) {
            ...
            if (
                defined(dbRoot.endSnippet) &&
                defined(dbRoot.endSnippet.model)
            ) {
            ...
            for (let i = 0; i < count; ++i) {
                ...
                if (defined(copyrightString)) {
                    providers[provider.providerId] = new Credit(
