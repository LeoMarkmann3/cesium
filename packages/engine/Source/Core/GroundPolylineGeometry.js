    ...
    for (i = 0; i < positionsLength - 1; i++) {
        ...
        if (
            ...
            if (groundPolylineGeometry.arcType === ArcType.GEODESIC) {
                ...
                c0 = ellipsoid.cartesianToCartographic(
                    p0,
                    cartographicScratch0,
                );
                ...
                intersectionCartographic =
                    rhumbLine.findIntersectionWithLongitude(
...
function projectNormal(
    ...
    let normalEndpoint = Cartesian3.add(
        position,
        normal,
    ...
    if (
        Math.abs(
            cartographic.longitude - normalEndpointCartographic.longitude,
        ) > CesiumMath.PI_OVER_TWO
    ) {
        flipNormal = true;
...
function generateGeometryAttributes(
    ...
    for (i = 0; i < segmentCount; i++) {
        const startBottom = Cartesian3.clone(
            endBottom,
            segmentStartBottomScratch,
        );
        ...
        startCartographic.longitude =
            cartographicsArray[cartographicsIndex + 1];
        ...
        if (compute2dAttributes) {
            const nudgeResult = nudgeCartographic(
                startCartographic,
                endCartographic,
            );
            start2D = projection.project(
                startCartographic,
            ...
            if (
                nudgeResult === 0 ||
                Cartesian3.dot(direction2D, Cartesian3.UNIT_Y) >
                    MITER_BREAK_SMALL
            ) {
                ...
                startGeometryNormal2D.y = CesiumMath.sign(
                    startCartographic.longitude -
                        Math.abs(endCartographic.longitude),
                );
        ...
        let startUp = Cartesian3.subtract(
            startTop,
            startBottom,
            startUpScratch,
        );
        ...
        startPlaneNormal = Cartesian3.normalize(
            startPlaneNormal,
            startPlaneNormal,
        );
        ...
        for (j = 0; j < 8; j++) {
            ...
            let texcoordNormalization =
                texcoordNormalization3DY * topBottomSide;
            ...
            if (compute2dAttributes) {
                ...
                texcoordNormalization =
                    texcoordNormalization2DY * topBottomSide;
                if (texcoordNormalization === 0.0 && topBottomSide < 0.0) {
                    texcoordNormalization = 9.0; // some value greater than 1.0
                }
    ...
    if (compute2dAttributes) {
        ...
        attributes.offsetAndRight2D =
            getVec4GeometryAttribute(offsetAndRight2D);
        attributes.startEndNormals2D =
            getVec4GeometryAttribute(startEndNormals2D);
