...
HeightmapTessellator.computeVertices = function (options) {
    ...
    if (!defined(rectangle)) {
        if (isGeographic) {
            ...
            geographicSouth =
                ...
                2.0 *
                    atan(
                        exp(-nativeRectangle.south * oneOverGlobeSemimajorAxis),
                    );
            ...
            geographicNorth =
                ...
                2.0 *
                    atan(
                        exp(-nativeRectangle.north * oneOverGlobeSemimajorAxis),
                    );
        }
    } else {
    ...
    const structure =
        options.structure ?? HeightmapTessellator.DEFAULT_STRUCTURE;
    const heightScale =
    ...
    for (let rowIndex = startRow; rowIndex < endRow; ++rowIndex) {
        ...
        if (!isGeographic) {
            latitude =
                piOverTwo -
                2.0 * atan(exp(-latitude * oneOverGlobeSemimajorAxis));
        } else {
        ...
        for (let colIndex = startCol; colIndex < endCol; ++colIndex) {
            ...
            let u =
                (longitude - geographicWest) /
                (geographicEast - geographicWest);
    ...
    const aaBox = new AxisAlignedBoundingBox(
        minimum,
        maximum,
        relativeToCenter,
    );
