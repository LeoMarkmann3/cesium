...
async function parseMetadataSuccess(terrainProviderBuilder, data, provider) {
    ...
    if (defined(availableTiles) && !defined(availabilityLevels)) {
        ...
        for (let level = 0; level < availableTiles.length; ++level) {
            ...
            const yTiles =
                terrainProviderBuilder.tilingScheme.getNumberOfYTilesAtLevel(
                    level,
                );
...
function createQuantizedMeshTerrainData(provider, buffer, level, x, y, layer) {
    ...
    const cartesian3Length =
        Float64Array.BYTES_PER_ELEMENT * cartesian3Elements;
    ...
    while (pos < view.byteLength) {
        ...
        if (
            ...
            layer.availabilityTilesLoaded.addAvailableTileRange(
                level,
                x,
                y,
                x,
    ...
    if (!defined(layerToUse) && unknownAvailability) {
        ...
        return availabilityPromise.then(() => {
            ...
            return new Promise((resolve) => {
                ...
                setTimeout(() => {
                    const promise = this.requestTileGeometry(
                        x,
                        y,
                        level,
                        request,
    ...
    return promise.then(function (buffer) {
        if (!defined(buffer)) {
            return Promise.reject(
                new RuntimeError("Mesh buffer doesn't exist."),
            );
        }
        ...
        if (
            ...
            if (!topLayer) {
                ...
                if (!defined(requestPromise)) {
                    ...
                    if (defined(requestPromise)) {
                        layer.availabilityPromiseCache[cacheKey] =
                            requestPromise;
