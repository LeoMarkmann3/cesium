    ...
    do {
        ...
        if (type === 0) {
            if (next === 0) {
                ...
                const previousEdge =
                    edgesOnPlane[edgeIndex - (abovePlane ? 1 : -1)];
...
PolygonGeometryLibrary.createGeometryFromPositions = function (
    ...
    let indices = PolygonPipeline.triangulate(
        polygon.positions2D,
        polygon.holes,
    );
    ...
    if (!perPositionHeight) {
        ...
        for (i = 0; i < length; i++) {
            ...
            if (arcType === ArcType.GEODESIC) {
                ...
                if (hasTexcoords) {
                    tempTexcoords =
                        PolygonGeometryLibrary.subdivideTexcoordLine(
                            t1,
                            t2,
                            p1,
                            p2,
                ...
                if (hasTexcoords) {
                    tempTexcoords =
                        PolygonGeometryLibrary.subdivideTexcoordRhumbLine(
                            t1,
                            t2,
                            ellipsoid,
                            p1,
