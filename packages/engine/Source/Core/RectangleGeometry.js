    ...
    if (
        ...
        for (let k = 0; k < textureCoordinates.length; k += 2) {
            textureCoordinates[k] =
                (textureCoordinates[k] - minX) / (maxX - minX);
...
function constructExtrudedRectangle(rectangleGeometry, computedOptions) {
    ...
    if (hasOffsets && !computeTopOffsets) {
        offsetValue =
            offsetAttributeValue === GeometryOffsetAttribute.ALL ? 1 : 0;
    ...
    if (!southCap) {
        for (i = area - width; i < area; i++) {
            ...
            if (shadowVolume) {
                extrudeNormalIndex += 3;
                wallExtrudeNormals[extrudeNormalIndex++] = topNormals[threeI];
                wallExtrudeNormals[extrudeNormalIndex++] =
                    topNormals[threeI + 1];
                wallExtrudeNormals[extrudeNormalIndex++] =
                    topNormals[threeI + 2];
            }
...
RectangleGeometry.createGeometry = function (rectangleGeometry) {
    ...
    if (extrude) {
        geometry = constructExtrudedRectangle(
            rectangleGeometry,
            computedOptions,
        );
