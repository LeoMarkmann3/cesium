...
GeometryPipeline.reorderForPreVertexCache = function (geometry) {
    ...
    if (defined(indices)) {
        ...
        const indicesOut = IndexDatatype.createTypedArray(
            numVertices,
            numIndices,
        );
        ...
        if (
            ...
            for (let i = 1; i < length; ++i) {
                const otherAttribute =
                    instances[i][propertyName].attributes[name];

                if (
                    !defined(otherAttribute) ||
                    attribute.componentDatatype !==
                        otherAttribute.componentDatatype ||
...
function combineGeometries(instances, propertyName) {
    ...
    for (name in attributes) {
        if (attributes.hasOwnProperty(name)) {
            ...
            for (i = 0; i < length; ++i) {
                sourceValues =
                    instances[i][propertyName].attributes[name].values;
    ...
    if (haveIndices) {
        ...
        for (i = 0; i < length; ++i) {
            ...
            offset += Geometry.computeNumberOfVertices(
        ...
        if (vertexNormalData.count > 0) {
            for (j = 0; j < vertexNormalData.count; j++) {
                Cartesian3.add(
                    normal,
                    normalsPerTriangle[
                        normalIndices[vertexNormalData.indexOffset + j]
                    ],
                    normal,
                );
            }
            ...
            if (
                ...
                Cartesian3.clone(
                    normalsPerTriangle[
                        normalIndices[vertexNormalData.indexOffset]
                    ],
                    normal,
                );
            }
...
GeometryPipeline.computeTangentAndBitangent = function (geometry) {
    ...
    for (i = 0; i < numIndices; i += 3) {
        ...
        const sdirx =
            (t2 * (vertices[i13] - ux) - t1 * (vertices[i23] - ux)) * r;
        const sdiry =
...
function splitLongitudePolyline(instance) {
    ...
    for (i = 0; i < length; i += 4) {
        ...
        if (defined(intersection)) {
            ...
            if (defined(colors)) {
                const c0 = Cartesian4.fromArray(
                    colors,
                    i0 * 4,
                    cartesian4Scratch0,
                );
                const c2 = Cartesian4.fromArray(
                    colors,
                    i2 * 4,
                    cartesian4Scratch0,
                );
            ...
            for (j = i * 2; j < i * 2 + 4 * 2; ++j) {
                currentAttributes.expandAndWidth.values.push(
                    expandAndWidths[j],
                );
                if (defined(texCoords)) {
