        ...
        for (let i = 0; i < length; i += 3) {
            ...
            if (vertexFormat.st) {
                if (!defined(flatTexcoords)) {
                    ...
                    const st = projectTo2d(
                        [p],
                        appendTextureCoordinatesCartesian2,
                    )[0];
                    Cartesian2.subtract(st, origin, st);

                    const stx = CesiumMath.clamp(
                        st.x / boundingRectangle.width,
                        0,
                        1,
                    );
                    const sty = CesiumMath.clamp(
                        st.y / boundingRectangle.height,
                        0,
                        1,
                    );
                    if (bottom) {
                        textureCoordinates[textureCoordIndex + bottomOffset2] =
                            stx;
                        textureCoordinates[
    ...
    if (closeTop || closeBottom) {
        ...
        if (closeTop && closeBottom) {
            ...
            if (vertexFormat.st && defined(textureCoordinates)) {
                ...
                topGeo.attributes.st.values = new Float32Array(
                    numPositions * 2,
                );
...
function expandRectangle(
    ...
    if (arcType !== ArcType.RHUMB) {
        ...
        if (t > 0.0 && t < 1.0) {
            ...
            const closestPolar = Stereographic.clone(
                lastPolar,
                scratchPolarClosest,
            );
...
PolygonGeometry.computeRectangleFromPositions = function (
    ...
    for (let i = 1; i < positionsLength; i++) {
        ...
        lastPolarPosition = Stereographic.clone(
            polarPosition,
            lastPolarPosition,
        );
    }
...
function createProjectTo2d(rectangle, outerPositions, ellipsoid) {
    return (positions, results) => {
        ...
        if (
            rectangle.height >= CesiumMath.PI ||
            ...
            if (defined(polygonGeometry._offsetAttribute)) {
                ...
                const offsetValue =
                    ...
                    GeometryOffsetAttribute.NONE
                        ? 0
                        : 1;
                const applyOffset = new Uint8Array(length / 3).fill(
                    offsetValue,
                );
