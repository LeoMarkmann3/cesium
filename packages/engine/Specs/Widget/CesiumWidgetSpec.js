...
describe(
    ...
    function () {
        ...
        it("can create, render, and destroy", function () {
            ...
            expect(widget.terrainProvider).toBeInstanceOf(
                EllipsoidTerrainProvider,
            );
        ...
        it("can set contextOptions", function () {
            ...
            expect(context.options.allowTextureFilterAnisotropic).toEqual(
                false,
            );
        ...
        it("stops tracking when tracked object is removed", function () {
            ...
            return pollToPromise(function () {
                ...
                return pollToPromise(function () {
                    ...
                    expect(widget.trackedEntity).toBeUndefined();
                    expect(widget.scene.camera.transform).toEqual(
                        Matrix4.IDENTITY,
                    );
                });
            });
        ...
        it("does not crash when tracking an object with a position property whose value is undefined.", function () {
            ...
            return pollToPromise(function () {
                widget.render();
                return widget.dataSourceDisplay.update(
                    widget.clock.currentTime,
                );
            }).then(function () {
                expect(
                    widget.scene.renderError.raiseEvent,
        ...
        it("sets the clock based on the first data source", function () {
            ...
            dataSource.clock.startTime =
                JulianDate.fromIso8601("2013-08-01T18:00Z");
            dataSource.clock.stopTime =
                JulianDate.fromIso8601("2013-08-21T02:00Z");
            ...
            return widget.dataSources.add(dataSource).then(function () {
                expect(widget.clock.startTime).toEqual(
                    dataSource.clock.startTime,
                );
                expect(widget.clock.stopTime).toEqual(
                    dataSource.clock.stopTime,
                );
                expect(widget.clock.currentTime).toEqual(
                    dataSource.clock.currentTime,
                );
                expect(widget.clock.clockRange).toEqual(
                    dataSource.clock.clockRange,
                );
                expect(widget.clock.clockStep).toEqual(
                    dataSource.clock.clockStep,
                );
                expect(widget.clock.multiplier).toEqual(
                    dataSource.clock.multiplier,
                );
            });
        });
        ...
        it("zoomTo zooms to TimeDynamicPointCloud with default offset when offset not defined", function () {
            widget = createCesiumWidget(container);
            return loadTimeDynamicPointCloud(widget).then(
                function (pointCloud) {
            ...
            return widget.dataSources
                ...
                .then(function () {
                    const preMixinListenerCount =
                        preMixinDataSource.entities.collectionChanged._listeners
                            .size;
                    const postMixinListenerCount =
                        postMixinDataSource.entities.collectionChanged
                            ._listeners.size;
