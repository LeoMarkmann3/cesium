...
describe(
    ...
    function () {
        ...
        beforeAll(function () {
            ...
            promises.push(
                Resource.fetchImage("./Data/Images/Gamma.png").then(
                    function (result) {
                        gammaImage = result;
                    },
                ),
            ...
            promises.push(
                Resource.fetchImage(
                    "./Data/Images/CustomColorProfile.png",
        ...
        it("faceNames returns an iterator over each of the faces by name", () => {
            ...
            for (const faceName of CubeMap.faceNames()) {
                expect(
                    Object.values(CubeMap.FaceName).includes(faceName),
                ).toBeTrue();
                count++;
            }
            ...
            const level0 = Object.values(CubeMap.FaceName).reduce(
                (level, faceName, index) => {
                    ...
                    const colorData = new Uint8Array(
                        [color, color, color, color].flat(),
                    );
                    level[faceName] = {
                        width: 2,
                        height: 2,
                        arrayBufferView: colorData,
                    };
                    return level;
                },
                {},
        ...
        it("fails to copy from the framebuffer (width)", function () {
            ...
            expect(function () {
                cubeMap.negativeY.copyFromFramebuffer(
                    0,
                    0,
                    0,
                    0,
