...
describe(
    ...
    function () {
        ...
        it("get maximumVaryingVectors", function () {
            expect(ContextLimits.maximumVaryingVectors).toBeGreaterThanOrEqual(
                8,
            );
        });
        ...
        it("get maximumVertexAttributes", function () {
            expect(
                ContextLimits.maximumVertexAttributes,
            ).toBeGreaterThanOrEqual(8);
        });
        ...
        it("get minimumAliasedLineWidth", function () {
            expect(ContextLimits.minimumAliasedLineWidth).toBeLessThanOrEqual(
                1,
            );
        });
        ...
        it("get minimumAliasedPointSize", function () {
            expect(ContextLimits.minimumAliasedPointSize).toBeLessThanOrEqual(
                1,
            );
        });
        ...
        it("gets the standard derivatives extension", function () {
            ...
            if (context.standardDerivatives) {
                fs +=
                    "  out_FragColor = vec4(dFdx(1.0), dFdy(1.0), 1.0, 1.0);\n";
            } else {
        ...
        it("get the maximum number of draw buffers", function () {
            if (context.drawBuffers) {
                expect(ContextLimits.maximumDrawBuffers).toBeGreaterThanOrEqual(
                    1,
                );
            } else {
        ...
        it("destroying Context destroys objects in cache", function () {
            const c = createContext();
            const destroyableObject = jasmine.createSpyObj(
                "destroyableObject",
                ["destroy"],
            );
