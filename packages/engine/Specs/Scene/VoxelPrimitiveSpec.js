...
describe(
    ...
    function () {
        ...
        it("statistics are updated when event listeners are assigned", async function () {
            ...
            expect(primitive.statistics.numberOfTilesWithContentReady).toEqual(
                1,
            );
        ...
        it("vertically exaggerates height bounds for ellipsoid-shaped voxels", function () {
            ...
            const globalTransform =
                provider.globalTransform ?? Matrix4.IDENTITY;
            ...
            spyOn(VoxelEllipsoidShape.prototype, "update").and.callFake(
                function (
                    ...
                    expect(minClippingBounds).toEqual(
                        expectedMinClippingBounds,
                    );
                    expect(maxClippingBounds).toEqual(
                        expectedMaxClippingBounds,
                    );
                },
            );
        ...
        it("applies vertical exaggeration to box-shaped voxels by scaling the model matrix", async function () {
            ...
            const scalar = Cartesian3.fromElements(
                1.0,
                1.0,
                exaggerationFactor,
            );
            ...
            const shapeTransform =
                boxProvider.shapeTransform ?? Matrix4.IDENTITY;
            const globalTransform =
                boxProvider.globalTransform ?? Matrix4.IDENTITY;
            const expectedCompoundMatrix = Matrix4.multiplyTransformation(
                globalTransform,
        ...
        it("accepts a new Custom Shader", async function () {
            ...
            expect(primitive.customShader).toBe(
