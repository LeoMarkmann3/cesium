...
describe("Scene/TerrainFillMesh", function () {
    ...
    beforeEach(function () {
        ...
        center =
            rootTiles[0].northeastChild.southwestChild.northeastChild
                .southwestChild;
    ...
    describe("updateFillTiles", function () {
        ...
        it("propagates fill tile edges to an unloaded tile", function () {
            ...
            return processor.process(tiles).then(function () {
                ...
                expect(
                    sw.eastTiles[0] === centerSE ||
                        se.westTiles[0] === centerSW,
                ).toBe(true);
                expect(
                    nw.eastTiles[0] === centerNE ||
                        ne.westTiles[0] === centerNW,
                ).toBe(true);

                expect(
                    sw.northTiles[0] === centerNW ||
                        nw.southTiles[0] === centerSW,
                ).toBe(true);
                expect(
                ...
                expect(
                    sw.northeastTile === centerNE ||
                        ne.southwestTile === centerSW,
                ).toBe(true);
                expect(
                    nw.southeastTile === centerSE ||
                        se.northwestTile === centerNW,
                ).toBe(true);
            });
        });
                    ...
                    mockTerrain
                        .requestTileGeometryWillSucceedWith(
                            new HeightmapTerrainData({
                                ...
                                createdByUpsampling: false,
                                buffer: new Float32Array([
                                    31.0, 32.0, 33.0, 38.0, 39.0, 40.0, 45.0,
                                    46.0, 47.0,
                                ]),
                            }),
                            south,
    ...
    describe("update", function () {
        ...
        it("uses adjacent edge heights", function () {
            return processor
                ...
                .then(function () {
                    const fill = (center.data.fill = new TerrainFillMesh(
                        center,
                    ));
        ...
        describe("correctly transforms texture coordinates across the anti-meridian", function () {
            ...
            it("western hemisphere to eastern hemisphere", function () {
                ...
                return processor
                    ...
                    .then(function () {
                        const fill = (easternHemisphere.data.fill =
                            new TerrainFillMesh(easternHemisphere));
            ...
            it("eastern hemisphere to western hemisphere", function () {
                ...
                return processor
                    .process([westernHemisphere, easternHemisphere])
                    .then(function () {
                        const fill = (westernHemisphere.data.fill =
                            new TerrainFillMesh(westernHemisphere));
    ...
    function expectVertexCount(fill, count) {
        expect(fill.mesh.vertices.length).toBe(
            count * fill.mesh.encoding.stride,
        );
    }
});
