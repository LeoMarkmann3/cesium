...
xdescribe(
    ...
    function () {
        ...
        beforeAll(function () {
            ...
            scene.camera.lookAt(
                center,
                new HeadingPitchRange(0.0, -1.57, 20.0),
            );
        ...
        it("setProperty with array value", function () {
            ...
            expect(batchTable.getProperty(1, "rooms")).toEqual([
                "room3",
                "room4",
            ]);
        });
        ...
        it("renders tileset with batch table hierarchy with no parents", function () {
            return checkBatchTableHierarchyNoParents(
                batchTableHierarchyNoParentsUrl,
            );
        });

        it("renders tileset with legacy batch table hierarchy (pre-version 1.0)", function () {
            return checkBatchTableHierarchy(
                batchTableHierarchyLegacyUrl,
                false,
            );
        });
        ...
        it("computes batchTableByteLength with a batch table hierarchy", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                const batchTable = content.batchTable;
                const hierarchySize =
                    batchTable._batchTableHierarchy.byteLength;
