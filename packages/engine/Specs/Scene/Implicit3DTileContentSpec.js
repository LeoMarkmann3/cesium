...
describe(
    ...
    function () {
        ...
        beforeEach(function () {
            ...
            mockPlaceholderTile = new Cesium3DTile(
                mockTileset,
                tilesetResource,
                {
                    geometricError: 400,
            ...
            scene.camera.lookAt(
                center,
                new HeadingPitchRange(0.0, -1.57, 26.0),
            );
        });
        ...
        it("loads subtree using JSON", async function () {
            ...
            for (let i = 0; i < tiles.length; i++) {
                expect(tiles[i].children.length).toEqual(
                    expectedChildrenCounts[i],
                );
            }
            expect(mockTileset.statistics.numberOfTilesTotal).toBe(
                tiles.length,
            );
        });
        ...
        it("handles deeper subtrees correctly", async function () {
            mockPlaceholderTile.implicitCoordinates =
            ...
            const rootBoundingVolume = [
                10, 0, 0, 256, 0, 0, 0, 256, 0, 0, 0, 256,
            ];
            ...
            expect(subtreeRootTile.implicitCoordinates).toEqual(
                parentCoordinates,
            );
            ...
            expect(subtreeRootTile.geometricError).toEqual(
        ...
        it("preserves tile extras", async function () {
            ...
            expect(mockPlaceholderTile.children[0].extras).toEqual(
                tileJson.extras,
            );
        });
        ...
        it("templates content URIs for each tile with content", async function () {
            ...
            for (let i = 0; i < tiles.length; i++) {
                ...
                if (contentAvailability[i]) {
                    expect(tiles[i]._contentResource.url).toEqual(
        ...
        describe("_deriveBoundingVolumeS2", function () {
            ...
            it("subdivides correctly using QUADTREE", function () {
                ...
                let result = deriveBoundingVolumeS2(
                    false,
                    parentTile,
                    0,
                    1,
            ...
            it("subdivides correctly using OCTREE", function () {
                implicitTilesetS2.subdivisionScheme =
        ...
        describe("_deriveBoundingRegion", function () {
            const deriveBoundingRegion =
                Implicit3DTileContent._deriveBoundingRegion;
            const simpleRegion = [
                0,
                0,
        ...
        describe("3DTILES_multiple_contents", function () {
            ...
            it("passes extensions through correctly (legacy)", function () {
                    ...
                    function (tilesetUrl) {
                        return originalLoadJson(tilesetUrl).then(
                            function (tilesetJson) {
                                ...
                                tilesetJson.extensionsUsed.push(
                                    "3DTILES_metadata",
                                );
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(
                        placeholderHeader.contents[0].extensions,
                    ).not.toBeDefined();
                    ...
                    for (let i = 0; i < childTiles.length; i++) {
                        ...
                        const childTileHeader = childTile._header;
                        expect(
                            childTileHeader.extensions["3DTILES_extension"],
                        ).toEqual(otherExtension);
        ...
        describe("metadata", function () {
            ...
            it("uses height semantics to adjust region bounding volumes", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    for (let i = 0; i < tiles.length; i++) {
                        ...
                        expect(tileRegion.minimumHeight).toBeGreaterThan(
                            minimumHeight,
                        );
                        expect(tileRegion.maximumHeight).toBeLessThan(
                            maximumHeight,
                        );
                    }
                });
            ...
            it("uses height semantics to adjust S2 bounding volumes", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    const implicitS2Volume =
                        placeholderTile.implicitTileset.boundingVolume
                    ...
                    for (let i = 0; i < tiles.length; i++) {
                        ...
                        expect(tileS2Volume.minimumHeight).toBeGreaterThan(
                            minimumHeight,
                        );
                        expect(tileS2Volume.maximumHeight).toBeLessThan(
                            maximumHeight,
                        );
                    }
                });
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    tiles.forEach(function (tile) {
                        ...
                        expect(halfWidth).toEqual(
                            rootHalfWidth / Math.pow(2, level),
                        );
                    });
                });
            });
            ...
            it("uses height semantics to adjust region semantic", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    for (let i = 0; i < tiles.length; i++) {
                        ...
                        expect(tileRegion.rectangle.south).toBeGreaterThan(
                            south,
                        );
            ...
            it("uses content bounding box from metadata semantics if present", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    tiles.forEach(function (tile) {
                        expect(
                            tile.contentBoundingVolume instanceof
                                TileBoundingSphere,
                        ).toBe(true);
                        expect(tile.contentBoundingVolume).not.toBe(
                            tile.boundingVolume,
                        );
                    });
                });
            ...
            it("uses content height semantics to adjust implicit region", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    for (let i = 0; i < tiles.length; i++) {
                        ...
                        expect(contentRegion.minimumHeight).toBeGreaterThan(
                            minimumHeight,
                        );
                        expect(contentRegion.maximumHeight).toBeLessThan(
                            maximumHeight,
                        );
                    }
                });
            ...
            it("uses content height semantics to adjust content region semantic", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    for (let i = 0; i < tiles.length; i++) {
                        ...
                        expect(contentRegion.rectangle.west).toBeGreaterThan(
                            west,
                        );
                        expect(contentRegion.rectangle.south).toBeGreaterThan(
                            south,
                        );
                        expect(contentRegion.rectangle.east).toBeLessThan(east);
                        expect(contentRegion.rectangle.north).toBeLessThan(
                            north,
                        );
                    }
                });
