...
describe(
    ...
    function () {
        ...
        beforeAll(function () {
            ...
            if (defined(scene.defaultView.globeDepth)) {
                globeDepthFramebuffer =
                    scene.defaultView.globeDepth.framebuffer;
            }
        ...
        beforeEach(function () {
            ...
            translucentPrimitive = new SpecPrimitive(
                primitive,
                Pass.TRANSLUCENT,
            );
        ...
        it("checks for support in the context on construction", function () {
            let translucentTileClassification =
                new TranslucentTileClassification({
                    depthTexture: true,
                });
        ...
        function expectResources(translucentTileClassification, toBeDefined) {
            ...
            expect(
                defined(translucentTileClassification._packFBO.framebuffer),
            ).toBe(toBeDefined);
            expect(
                defined(
                    translucentTileClassification._opaqueDepthStencilTexture,
                ),
            ).toBe(toBeDefined);
            expect(
                defined(
            ...
            expect(
                defined(
                    translucentTileClassification._translucentDepthStencilTexture,
                ),
            ).toBe(toBeDefined);
            expect(
                defined(
                    translucentTileClassification._packFBO.getColorTexture(),
                ),
            ).toBe(toBeDefined);
            expect(
                defined(translucentTileClassification._packDepthCommand),
            ).toBe(toBeDefined);
            expect(
                defined(translucentTileClassification._accumulateCommand),
            ).toBe(toBeDefined);
            expect(
                defined(translucentTileClassification._compositeCommand),
            ).toBe(toBeDefined);
        ...
        it("creates resources on demand", function () {
            const translucentTileClassification =
                new TranslucentTileClassification(context);
        ...
        it("draws translucent commands into a buffer for depth", function () {
            ...
            const packedDepthFBO =
                translucentTileClassification._packFBO.framebuffer;
            ...
            expect(translucentTileClassification.hasTranslucentDepth).toBe(
                true,
            );
        ...
        it("composites classification into a buffer", function () {
            ...
            const red =
                Math.round(pixelsToComposite[0]) + preCompositePixels[0];
            const green =
                Math.round(pixelsToComposite[1]) + preCompositePixels[1];
            const blue =
                Math.round(pixelsToComposite[2]) + preCompositePixels[2];
