...
describe(
    ...
    function () {
        ...
        beforeEach(async function () {
            ...
            camera = scene.camera;
            camera.position = Cartesian3.fromElements(
                -6378000,
                -6378000,
                -6378000,
            );
        ...
        it("recomputes bounding volume when shape moves", function () {
            ...
            expect(newOrientedBoundingBox.center.equals(translation)).toBe(
                true,
            );
        });
        ...
        it("computes screen space error for root tile", function () {
            ...
            let distanceToCamera = Math.sqrt(
                rootNode.orientedBoundingBox.distanceSquaredTo(
                    camera.positionWC,
                ),
            );
        ...
        it("shows texture memory allocation statistic", function () {
            expect(traversal.textureMemoryByteLength).toBe(
                textureMemoryByteLength,
            );
            traversal.destroy();
