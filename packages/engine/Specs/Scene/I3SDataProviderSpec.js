...
describe("Scene/I3SDataProvider", function () {
    ...
    it("constructs I3SDataProvider with BSL", async function () {
        ...
        expect(
            testProvider.sublayers[0].sublayers[0]._parent.modelName,
        ).toEqual("FullModel");
        ...
        expect(
            testProvider.sublayers[0].sublayers[1]._parent.modelName,
        ).toEqual("FullModel");
        expect(testProvider.sublayers[0].sublayers[0].sublayers.length).toEqual(
            3,
        );
        expect(
            testProvider.sublayers[0].sublayers[0].sublayers[0].name,
        ).toEqual("SubCat1");
        expect(
        ...
        expect(
            testProvider.sublayers[0].sublayers[0].sublayers[1].name,
        ).toEqual("SubCat2");
        expect(
        ...
        expect(
            testProvider.sublayers[0].sublayers[0].sublayers[2].name,
        ).toEqual("SubCat3");
        expect(
        ...
        expect(testProvider.sublayers[0].sublayers[1].sublayers.length).toEqual(
            0,
        );
    });
    ...
    it("wraps updateForPass", async function () {
        ...
        expect(
            testProvider._layers[1]._tileset.updateForPass,
        ).toHaveBeenCalledWith(frameState, passState);
    });
    ...
    it("loadJson rejects error response", async function () {
        ...
        const resource = Resource.createIfNeeded("mockJsonUri");
        await expectAsync(
            I3SDataProvider.loadJson(resource),
        ).toBeRejectedWithError(RuntimeError, mockErrorResponse.error);
    });
        ...
        spyOn(I3SDataProvider, "_fetchJson").and.callFake(function (resource) {
            if (
                resource.url.includes(
                    "mockProviderUrl/layers/0/mockRootNodeUrl/?",
                ) ||
                resource.url.includes(
                    "mockProviderUrl/layers/1/mockRootNodeUrl/?",
                )
            ) {
