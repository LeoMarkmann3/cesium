...
describe("Scene/I3SNode", function () {
    ...
    const customAttributes = {
        ...
        positions: new Float32Array([
            -20, -20, 0, 20, 0, 0, -20, 0, 0, -20, -20, 0, 20, -20, 0, 20, 0, 0,
            -20, 0, 0, 20, 20, 0, -20, 20, 0, -20, 0, 0, 20, 0, 0, 20, 20, 0,
        ]),
        ...
        spyOn(rootNode._dataProvider, "_loadBinary").and.callFake(
            function (resource) {
                return new Promise(function (resolve, reject) {
                    let resultBuffer = "";
                    if (
                        resource.url.includes("f_0/") ||
                        resource.url.includes("f_1/")
                    ) {
    ...
    it("loads geometry from node pages", async function () {
        ...
        return nodeWithMesh
            ...
            .then(function (result) {
                ...
                expect(
                    nodeWithMesh.geometryData[0].resource.queryParameters
                        .testQuery,
                ).toEqual("test");

                //Expect geometry 1 to have been picked because geometry 0 didn't have all the required properties
                expect(
                    nodeWithMesh._dataProvider._loadBinary,
                ).toHaveBeenCalledWith(nodeWithMesh.geometryData[0].resource);
                expect(
                ...
                expect(nodeWithMesh.geometryData[0].data).toEqual(
                    new ArrayBuffer(),
                );
            });
    });
    ...
    it("generate geometry from node pages", async function () {
        ...
        spyOn(
            nodeWithTexturedMesh._dataProvider,
            "_loadBinary",
        ).and.returnValue(Promise.resolve(new ArrayBuffer()));
        ...
        return nodeWithTexturedMesh
            ...
            .then(function (result) {
                const rawGltf =
                    nodeWithTexturedMesh.geometryData[0]._generateGltf(
    ...
    it("generate geometry from node pages with transparent material when alpha mode is defined", async function () {
        ...
        mockI3SLayerWithNodePages._data.materialDefinitions[0].alphaMode =
            "blend";
        mockI3SLayerWithNodePages._data.materialDefinitions[1].alphaMode =
            "blend";
        const nodeWithTexturedMesh = new I3SNode(
            mockI3SLayerWithNodePages,
            1,
    ...
    it("generate geometry from node pages without material", async function () {
        ...
        return nodeWithTexturedMesh
            ...
            .then(function (result) {
                ...
                delete copyI3sGeometryData.meshData.meshes[0].primitives[0]
                    .material;
                const rawGltf =
                    nodeWithTexturedMesh.geometryData[0]._generateGltf(
    ...
    it("creates 3d tile content", async function () {
        ...
        spyOn(nodeWithMesh, "_loadFeatureData").and.returnValue(
            Promise.all([]),
        );
    ...
    it("requests content", async function () {
        ...
        return rootNode
            ...
            .then(function () {
                expect(childNode.tile._contentResource._url).toEqual(
                    "mockGlbUrl",
                );
            });
    });
    ...
    it("can filter by attributes", async function () {
        ...
        return rootNode
            ...
            .then(function (result) {
                ...
                childNode._layer._data.attributeStorageInfo = [
                    { name: "BldgLevel" },
                ];
            ...
            .then(function () {
                expect(content.batchTable.features).toEqual([
                    false,
                    false,
                    true,
                ]);
