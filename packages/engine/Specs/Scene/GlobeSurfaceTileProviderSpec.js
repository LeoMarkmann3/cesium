...
describe(
    ...
    function () {
        ...
        describe(
            ...
            function () {
                it("throws if a terrainProvider is not provided", function () {
                    ...
                    expect(
                        constructWithoutTerrainProvider,
        ...
        describe(
            ...
            function () {
                ...
                it("adding a layer adds it to all tiles after update", async function () {
                    ...
                    // Add another layer
                    const layer2 =
                        scene.imageryLayers.addImageryProvider(provider2);
                ...
                it("moving a layer moves the corresponding TileImagery instances on every tile", async function () {
                        ...
                        function (tile) {
                            ...
                            for (let i = 0; i < tile.data.imagery.length; ++i) {
                                ...
                                if (imagery.imageryLayer === layer1) {
                                    indexOfFirstLayer1 = Math.min(
                                        indexOfFirstLayer1,
                                        i,
                                    );
                                    indexOfLastLayer1 = i;
                                } else {
                                    expect(imagery.imageryLayer).toEqual(
                                        layer2,
                                    );
                                    indexOfFirstLayer2 = Math.min(
                                        indexOfFirstLayer2,
                                        i,
                                    );
                                }
                            }
                            expect(indexOfFirstLayer1).toBeLessThan(
                                indexOfFirstLayer2,
                            );
                            expect(indexOfLastLayer1).toBeLessThan(
                                indexOfFirstLayer2,
                            );
                        },
                    );
                    ...
                    forEachRenderedTile(
                        ...
                        function (tile) {
                            ...
                            expect(indexOfFirstLayer2).toBeLessThan(
                                indexOfFirstLayer1,
                            );
                            expect(indexOfLastLayer2).toBeLessThan(
                                indexOfFirstLayer1,
                            );
                        },
                    );
                ...
                it("calling _reload adds a callback per layer per tile", async function () {
                    ...
                    forEachRenderedTile(
                        ...
                        function (tile) {
                            ...
                            expect(
                                Object.keys(tile._loadedCallbacks).length,
                            ).toBe(0);
                        },
                    );
        ...
        it("renders even if imagery root tiles fail to load", function () {
            ...
            const providerWithInvalidRootTiles =
        ...
        it("passes layer adjustment values as uniforms", async function () {
            ...
            for (let i = 0; i < commandList.length; ++i) {
                ...
                if (
                    !defined(uniforms) ||
                    !defined(uniforms.u_dayTextureAlpha)
                ) {
                    continue;
                }
                ...
                expect(uniforms.u_dayTextureOneOverGamma()).toEqual([
                    1.0 / 0.321,
                ]);
        ...
        it("can render more imagery layers than the available texture units", async function () {
            ...
            for (
                let i = 0;
                i < ContextLimits.maximumTextureImageUnits + 1;
                ++i
            ) {
                scene.imageryLayers.addImageryProvider(provider);
            }
            ...
            for (let i = 0; i < commandList.length; ++i) {
                ...
                if (command.owner instanceof QuadtreeTile) {
                    ...
                    if (!defined(drawCommandsPerTile[key])) {
                        ...
                        expect(command.uniformMap.u_initialColor().w).toEqual(
                            0.0,
                        );
                    }
            ...
            for (const tileID in drawCommandsPerTile) {
                if (drawCommandsPerTile.hasOwnProperty(tileID)) {
                    ++tileCount;
                    expect(drawCommandsPerTile[tileID]).toBeGreaterThanOrEqual(
                        2,
                    );
                }
            }
        ...
        it("clipping planes selectively disable rendering globe surface", function () {
            ...
            return updateUntilDone(scene.globe).then(function () {
                ...
                const clipPlane = new ClippingPlane(
                    Cartesian3.UNIT_Z,
                    -10000.0,
                );
        ...
        it("cartographicLimitRectangle defaults to Rectangle.MAX_VALUE", function () {
            ...
            expect(
                scene.globe.cartographicLimitRectangle.equals(
                    Rectangle.MAX_VALUE,
                ),
            ).toBe(true);
        });
        ...
        it("cartographicLimitRectangle may cross the antimeridian", function () {
            ...
            return updateUntilDone(scene.globe)
                .then(function () {
                    unculledCommandCount = scene.frameState.commandList.length;
                    scene.globe.cartographicLimitRectangle =
                        Rectangle.fromDegrees(179, -2, -179, -1);
                    return updateUntilDone(scene.globe);
                })
                .then(function () {
                    expect(unculledCommandCount).toBeGreaterThan(
            ...
            return updateUntilDone(scene.globe).then(function () {
                forEachRenderedTile(
                    ...
                    function (tile) {
                        ...
                        expect(encoding.hasGeodeticSurfaceNormals).toEqual(
                            false,
                        );
                ...
                return updateUntilDone(scene.globe).then(function () {
                    forEachRenderedTile(
                        ...
                        function (tile) {
                            ...
                            expect(boundingSphere.radius).toBeGreaterThan(
                                7000000.0,
                            );
                        },
                    );
                    ...
                    return updateUntilDone(scene.globe).then(function () {
                        forEachRenderedTile(
                            ...
                            function (tile) {
                                ...
                                const encoding = surfaceTile.mesh.encoding;
                                const boundingSphere =
                                    surfaceTile.tileBoundingRegion
                                        .boundingSphere;
                                expect(encoding.exaggeration).toEqual(1.0);
                                expect(
                                    encoding.hasGeodeticSurfaceNormals,
                                ).toEqual(false);
                                expect(boundingSphere.radius).toBeLessThan(
                                    7000000.0,
                                );
                            },
                        );
