    ...
    () => {
        ...
        describe("points", () => {
            ...
            it("picks batched points with batch table", () => {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(scene).toPickAndCall((result) => {
                        expect(result).toBeDefined();
                        expect(result).toBeInstanceOf(Cesium3DTilePointFeature);
                        expect(result.getProperty("name")).toEqual(
                            "upper left",
                        );
                    });
                    camera.lookAt(
            ...
            it("sets the heightReference for Vector3DTilePoints", async () => {
                ...
                expect(vectorTilePoint._heightReference).toEqual(
                    heightReference,
                );
            });
        });
            ...
            describe("getPolylinePositions", () => {
                it("gets polyline positions", function () {
                    return Cesium3DTilesTester.loadTileset(
                        ...
                        const polylinePositions =
                            content.getPolylinePositions(0);
                ...
                it("getPolylinePositions returns undefined if there are no positions associated with the given batchId", function () {
                    return Cesium3DTilesTester.loadTileset(
                        ...
                        const content = tileset.root.content;
                        const polylinePositions =
                            content.getPolylinePositions(1);
        ...
        it("throws with invalid version", async function () {
            ...
            await expectAsync(
                Cesium3DTilesTester.createContentForMockTile(
                    arrayBuffer,
                    "vctr",
                ),
