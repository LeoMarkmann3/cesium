...
describe(
    ...
    function () {
        ...
        const gltfContentUrl =
            "Data/Cesium3DTiles/GltfContent/glTF/tileset.json";
        ...
        function setZoom(distance) {
            ...
            scene.camera.lookAt(
                center,
                new HeadingPitchRange(0.0, -1.57, distance),
            );
        }
        ...
        it("fromUrl throws with invalid tileset JSON file", async function () {
            await expectAsync(
                Cesium3DTileset.fromUrl("invalid.json"),
            ).toBeRejected();
        });
        ...
        it("hasExtension returns true if the tileset JSON file uses the specified extension", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                expect(
                    tileset.hasExtension("3DTILES_batch_table_hierarchy"),
                ).toBe(true);
                expect(
                    tileset.hasExtension("3DTILES_nonexistant_extension"),
                ).toBe(false);
            });
        });
        ...
        function checkPointAndFeatureCounts(
            tileset,
            features,
            points,
            triangles,
            ...
            return Cesium3DTilesTester.waitForTilesLoaded(scene, tileset).then(
                function () {
                    expect(statistics.numberOfFeaturesSelected).toEqual(
                        features,
                    );
                    ...
                    expect(statistics.numberOfTrianglesSelected).toEqual(
                        triangles,
                    );
                ...
                function (tileset) {
                    ...
                    return Cesium3DTilesTester.waitForTilesLoaded(
                        ...
                        return Cesium3DTilesTester.waitForTilesLoaded(
                            ...
                            tileset.root.content.getFeature(0).color =
                                Color.RED;
                            scene.renderForSpecs();
                            expect(statistics.geometryByteLength).toEqual(
                                singleTileGeometryMemory * tilesLength,
        ...
        async function testDynamicScreenSpaceError(url, distance) {
            ...
            scene.camera.lookAt(
                center,
                new HeadingPitchRange(0.0, 0.0, distance),
            );
        ...
        it("replacement refinement - selects root when sse is not met and subtree is not refinable (3)", async function () {
            ...
            await pollToPromise(() => {
                scene.renderForSpecs();
                return (
                    root.children[0].contentFailed ||
                    root.children[0].contentReady
                );
            });
        ...
        it("replacement refinement - refines if descendant is empty leaf tile", function () {
            ...
            spyOn(Cesium3DTileset, "loadJson").and.callFake(
                function (tilesetUrl) {
                    return originalLoadJson(tilesetUrl).then(
        ...
        describe("children bound union optimization", function () {
            it("does not select visible tiles with invisible children", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    scene.camera.lookAt(
                        center,
                        new HeadingPitchRange(0.0, 1.57, 1.0),
                    );
            ...
            it("does not select external tileset whose root has invisible children", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(
                        tileset.statistics.numberOfTilesCulledWithChildrenUnion,
                    ).toBe(1);
                });
            });
        ...
        it("set tile color", function () {
            return Cesium3DTilesTester.loadTileset(scene, noBatchIdsUrl).then(
                function (tileset) {
                    // Get initial color
                    let color;
                    Cesium3DTilesTester.expectRender(
                        scene,
                        tileset,
                        function (rgba) {
                            color = rgba;
        ...
        it("debugWireframe", function () {
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl, {
                ...
                let i;
                for (i = 0; i < length; ++i) {
                    expect(commands[i].primitiveType).toEqual(
                        PrimitiveType.LINES,
                    );
                }
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    ...
                    const expected =
                        ...
                        `Geometry Memory: ${(0.007).toLocaleString()}\n` +
                        "Url: parent.b3dm";
                    expect(tileset._tileDebugLabels.get(0).text).toEqual(
                        expected,
                    );
        ...
        it("does not request tiles when picking", function () {
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    viewRootOnly();
                    scene.pickForSpecs();
                    expect(tileset._statistics.numberOfPendingRequests).toEqual(
                        0,
                    );
                    scene.renderForSpecs();
                    expect(tileset._statistics.numberOfPendingRequests).toEqual(
        ...
        it("does not process tiles when picking", async function () {
            ...
            await pollToPromise(() => {
                scene.renderForSpecs();
                return (
                    tileset.root._contentState ===
                    Cesium3DTileContentState.PROCESSING
                );
            });

            const spy = spyOn(
                Cesium3DTile.prototype,
                "process",
            ).and.callThrough();
            scene.pickForSpecs();
        ...
        xit("does not request tiles when the request scheduler is full", function () {
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl, {
                ...
                expect(tileset._statistics.numberOfAttemptedRequests).toEqual(
                    1,
                );
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    ...
                    return Cesium3DTilesTester.waitForTilesLoaded(
                        ...
                        // Root is loaded
                        expect(spyUpdate.calls.count()).toEqual(1);
                        expect(spyUpdate.calls.argsFor(0)[0]).toBe(
                            tileset.root,
                        );
        ...
        it("handle else case when applying conditional color style to a tileset", function () {
            ...
            return Cesium3DTilesTester.loadTileset(
                ...
                scene.renderForSpecs();
                expect(tileset.root.content.getFeature(0).color).toEqual(
                    Color.WHITE,
                );
                expect(tileset.root.content.getFeature(1).color).toEqual(
                    Color.BLACK,
                );
            });
        });
        ...
        it("loads style from uri", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                // ${id} < 10 will always evaluate to true
                return Cesium3DTileStyle.fromUrl(styleUrl).then(
                    function (style) {
                        tileset.style = style;
                        expect(scene).toRenderAndCall(function (rgba) {
                            expect(rgba[0]).toBeGreaterThan(0);
        ...
        async function testColorBlendMode(url) {
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                expect(rgba[0])
                    .withContext("starting red .r")
                    .toBeGreaterThan(190);
                expect(rgba[1])
                    .withContext("starting red .g")
                    .toEqualEpsilon(118, 1);
                expect(rgba[2])
                    .withContext("starting red .b")
                    .toEqualEpsilon(118, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                ...
                expect(rgba[1])
                    .withContext("hl dark yellow .g")
                    .toEqualEpsilon(58, 1);
                expect(rgba[2])
                    .withContext("hl dark yellow .b")
                    .toEqualEpsilon(0, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                ...
                expect(rgba[0])
                    ...
                    .toBeLessThan(sourceRed);
                expect(rgba[1])
                    .withContext("hl yellow+alpha .g")
                    .toEqualEpsilon(80, 1);
                expect(rgba[2])
                    .withContext("hl yellow+alpha .b")
                    .toEqualEpsilon(0, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                ...
                replaceGreen = rgba[1];
                expect(rgba[0])
                    .withContext("replace yellow .r")
                    .toBeGreaterThan(100);
                expect(rgba[0])
                    .withContext("replace yellow .r")
                    .toBeLessThan(255);
                expect(rgba[1])
                    .withContext("replace yellow .g")
                    .toBeGreaterThan(100);
                expect(rgba[1])
                    .withContext("replace yellow .g")
                    .toBeLessThan(255);
                expect(rgba[2])
                    .withContext("replace yellow .b")
                    .toEqualEpsilon(62, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                ...
                expect(rgba[2])
                    ...
                    .toEqualEpsilon(80, 1);
                expect(rgba[3])
                    .withContext("replace yellow+alpha .a")
                    .toEqual(255);
            });
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                ...
                expect(rgba[1])
                    .withContext("mix yellow .g")
                    .toBeLessThan(replaceGreen);
                expect(rgba[2])
                    .withContext("mix yellow .b")
                    .toEqualEpsilon(96, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                ...
                expect(rgba[1])
                    .withContext("mix blend 0.25 .g")
                    .toBeGreaterThan(0);
                expect(rgba[1])
                    .withContext("mix blend 0.25 .g")
                    .toBeLessThan(mixGreen);
                expect(rgba[2])
                    .withContext("mix blend 0.25 .b")
                    .toEqualEpsilon(108, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                expect(rgba[0])
                    .withContext("mix blend 0.0 .r")
                    .toEqual(sourceRed);
                expect(rgba[1])
                    .withContext("mix blend 0.0 .g")
                    .toEqualEpsilon(118, 1);
                expect(rgba[2])
                    .withContext("mix blend 0.0 .b")
                    .toEqualEpsilon(118, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                expect(rgba[0])
                    .withContext("mix blend 1.0 .r")
                    .toEqual(replaceRed);
                expect(rgba[1])
                    .withContext("mix blend 1.0 .g")
                    .toEqual(replaceGreen);
                expect(rgba[2])
                    .withContext("mix blend 1.0 .b")
                    .toEqualEpsilon(62, 1);
            ...
            expect(renderOptions).toRenderAndCall(function (rgba) {
                expect(rgba[0])
                    .withContext("mix yellow+alpha .r")
                    .toBeGreaterThan(0);
                expect(rgba[1])
                    .withContext("mix yellow+alpha .g")
                    .toBeGreaterThan(0);
                expect(rgba[2])
        ...
        it("tileUnload event is raised", function () {
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    ...
                    expect(spyUpdate.calls.argsFor(0)[0]).toBe(
                        tileset.root.children[0],
                    );
                    expect(spyUpdate.calls.argsFor(1)[0]).toBe(
                        tileset.root.children[1],
                    );
                    expect(spyUpdate.calls.argsFor(2)[0]).toBe(
                        tileset.root.children[2],
                    );
                    expect(spyUpdate.calls.argsFor(3)[0]).toBe(
                        tileset.root.children[3],
                    );
                },
            );
        });
        ...
        it("marks tileset as mixed when tiles have nonzero selection depth", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                expect(statistics.numberOfTilesWithContentReady).toEqual(2);
                expect(
                    tileset.root.children[0].children[0].children[3]
                        ._selectionDepth,
                ).toEqual(1);
        ...
        it("creates duplicate backface commands", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                expect(
                    isSelected(
                        tileset,
                        root.children[0].children[0].children[3],
                    ),
                ).toBe(true);
                expect(
                    root.children[0].children[0].children[3]._finalResolution,
                ).toBe(true);
                expect(tileset.hasMixedContent).toBe(true);
        ...
        it("does not create duplicate backface commands if no selected descendants", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                expect(
                    isSelected(
                        tileset,
                        root.children[0].children[0].children[2],
                    ),
                ).toBe(false);
                expect(
                    isSelected(
        ...
        it("does not add stencil clear command or backface commands when fully resolved", function () {
            ...
            return Cesium3DTilesTester.loadTileset(
                ...
                for (let i = 0; i < length; ++i) {
                    ...
                    expect(command.renderState.cull.face).not.toBe(
                        CullFace.FRONT,
                    );
                }
            });
        ...
        it("tile expires", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                expect(tile.contentAvailable).toBe(true);
                expect(tile._contentState).toBe(
                    Cesium3DTileContentState.LOADING,
                );
        ...
        it("tile expiration date", function () {
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    ...
                    // Stays in the expired state until the request goes through
                    const originalMaxmimumRequests =
                        RequestScheduler.maximumRequests;
                    RequestScheduler.maximumRequests = 0; // Artificially limit Request Scheduler so the request won't go through
                    scene.renderForSpecs();
        ...
        it("destroys attached ClippingPlaneCollections and ClippingPlaneCollections that have been detached", function () {
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    const clippingPlaneCollection1 =
                    ...
                    const clippingPlaneCollection2 =
                        new ClippingPlaneCollection({
                            planes: [
                                new ClippingPlane(
                                    Cartesian3.UNIT_Z,
        ...
        it("clipping planes cull hidden content", function () {
            return Cesium3DTilesTester.loadTileset(scene, tilesetUrl).then(
                function (tileset) {
                    let visibility = tileset.root.contentVisibility(
                        scene.frameState,
                    );
                    ...
                    visibility = tileset.root.contentVisibility(
                        scene.frameState,
                    );
        ...
        it("clippingPlanesOriginMatrix matches root tile bounding sphere", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                expect(
                    Matrix4.equals(offsetMatrix, boundingSphereEastNorthUp),
                ).toBe(true);
        ...
        describe("updateForPass", function () {
            it("updates for pass", function () {
                ...
                const passCullingVolume =
                    passCamera.frustum.computeCullingVolume(
        ...
        describe("most detailed height queries", function () {
            it("tileset is offscreen", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    return sampleHeightMostDetailed(cartographics).then(
                        function () {
                            ...
                            const statisticsMostDetailedPick =
                                tileset._statisticsPerPass[
                                    Cesium3DTilePass.MOST_DETAILED_PICK
                                ];
                            ...
                            expect(
                                statisticsMostDetailedPick.numberOfCommands,
                            ).toBe(1);
                            expect(
                            ...
                            expect(
                                statisticsMostDetailedPick.visited,
                            ).toBeGreaterThan(1);
                            expect(
                                statisticsMostDetailedPick.numberOfTilesTotal,
                            ).toBe(21);
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    return sampleHeightMostDetailed(cartographics).then(
                        function () {
                            ...
                            expect(statisticsRender.selected).toBeGreaterThan(
                                0,
                            );
                        },
                    );
            ...
            it("drill picks multiple features when tileset uses additive refinement", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    return drillPickFromRayMostDetailed(ray).then(
                        function (results) {
                            expect(results.length).toBe(3);
                            expect(
                                results[0].object.content.url.indexOf(
                                    "0_0_0.b3dm",
                                ) > -1,
                            ).toBe(true);
                            expect(
                                results[1].object.content.url.indexOf(
                                    "1_1_1.b3dm",
                                ) > -1,
                            ).toBe(true);
                            expect(
                                results[2].object.content.url.indexOf(
                                    "2_4_4.b3dm",
                                ) > -1,
                            ).toBe(true);
                        },
        ...
        it("cancels out-of-view tiles", function () {
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUniform).then(
                function (tileset) {
                    ...
                    viewAllTiles();
                    scene.renderForSpecs();
                    const requestedTilesInFlight =
                        tileset._requestedTilesInFlight;
                    const requestedTilesInFlightLength =
                        requestedTilesInFlight.length;
        ...
        it("sorts requests by priority", function () {
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUniform).then(
                function (tileset) {
                    ...
                    expect(lastPriority).not.toEqual(
                        requestedTilesInFlight[0]._priority,
        ...
        it("loads deferred requests only after time delay.", function () {
            ...
            return Cesium3DTilesTester.loadTileset(
                ...
                return pollToPromise(function () {
                    ...
                    expect(
                        tileset._requestedTilesInFlight[0].priorityDeferred,
                    ).toBe(true);
        ...
        it("preloads tiles", function () {
            ...
            return Cesium3DTilesTester.loadTileset(scene, tilesetUniform).then(
                function (tileset) {
                    spyOn(Camera.prototype, "canPreloadFlight").and.callFake(
                        function () {
                            return true;
                        },
                    );
        ...
        it("displays copyrights for glTF content in sorted order", function () {
            return Cesium3DTilesTester.loadTileset(
                ...
                const mostFrequentCopyright = new Credit(
                    "Most Frequent Copyright",
                );
                const secondRepeatedCopyright = new Credit(
                    "Second Repeated Copyright",
                );
                const lastRepeatedCopyright = new Credit(
                    "Last Repeated Copyright",
                );
                ...
                const creditContainer =
                    creditDisplay._lightboxCredits.childNodes[2];
        ...
        describe("select feature IDs", function () {
            ...
            it("instanceFeatureIdLabel sets from string", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(tileset.instanceFeatureIdLabel).toBe(
                        "instanceFeatureId_0",
                    );
        ...
        describe("metadata", function () {
            ...
            it("loads tileset metadata", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(
                        tilesetMetadata.getPropertyBySemantic("DATE_ISO_8601"),
                    ).toBe(tilesetProperties.date);
                    expect(
                        tilesetMetadata.getProperty("centerCartographic"),
                    ).toEqual(
            ...
            it("loads explicit tileset with tile metadata", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    tiles.forEach(function (tile) {
                        ...
                        expect(
                            metadata.getProperty("areaPercentage"),
                        ).toEqualEpsilon(
            ...
            it("loads implicit tileset with tile metadata", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    const tiles = [transcodedRoot].concat(
                        transcodedRoot.children,
                    );
                    ...
                    for (i = 0; i < tileCount; i++) {
                        ...
                        if (!subtree.tileIsAvailableAtIndex(index)) {
                            expect(
                                metadata.getProperty("quadrant"),
                            ).not.toBeDefined();
                            expect(
                                metadata.getProperty("color"),
                            ).not.toBeDefined();
                        } else {
            ...
            it("loads explicit tileset with multiple contents with metadata", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(
                        batchedContentMetadata.getProperty("highlightColor"),
                    ).toEqual(new Cartesian3(0, 0, 255));
                    expect(
                        batchedContentMetadata.getProperty("author"),
                    ).toEqual("Cesium");
                    expect(
                        batchedContentMetadata.hasProperty("numberOfInstances"),
                    ).toBe(false);
                    ...
                    expect(
                        instancedContentMetadata.getProperty(
                            "numberOfInstances",
                        ),
                    ).toEqual(50);
                    expect(
                        instancedContentMetadata.getProperty("author"),
                    ).toEqual("Sample Author");
                    expect(
                        instancedContentMetadata.hasProperty("highlightColor"),
                    ).toBe(false);
                });
            });
            ...
            it("loads implicit tileset with multiple contents with metadata", function () {
                ...
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(buildingMetadataTable.hasProperty("height")).toBe(
                        true,
                    );
                    expect(buildingMetadataTable.hasProperty("color")).toBe(
                        true,
                    );
