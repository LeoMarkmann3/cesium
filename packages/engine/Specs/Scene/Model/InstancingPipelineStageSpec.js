    ...
    function () {
        ...
        it("creates instancing matrices vertex attributes when ROTATION is present", function () {
            return loadGltf(boxInstanced).then(function (gltfLoader) {
                ...
                expect(
                    renderResources.model.statistics.geometryByteLength,
                ).toBe(0);
            });
        });
        ...
        it("creates instancing matrices vertex attributes for 2D", function () {
            return loadGltf(boxInstanced, {
                ...
                expect(renderResources.model._pipelineResources.length).toEqual(
                    0,
                );
        ...
        it("correctly creates transform matrices", function () {
            return loadGltf(boxInstanced).then(function (gltfLoader) {
                ...
                const expectedTransformsTypedArray = new Float32Array([
                    ...
                    -0.7071067094802856, 2, 0, 0.49497467279434204,
                    0.7071067690849304, 0, 0.7071068286895752,
                    4.174155421310388e-8, 0.3535534143447876, -2, 0.5,
                    0.7071068286895752, -0.2500000298023224, -2,
                    -0.5000000596046448, 0.7071068286895752, 0.25, 0, 0.375,
                    -0.10000001639127731, 0.3535534143447876, 2,
                    0.6401650905609131, 0.029289301484823227,
                    -0.2500000298023224, -2, 0.10983504354953766,
                    0.1707106977701187, 0.25, 0, 0.4898979365825653,
                    -0.3674234449863434, 0.44999992847442627, 2,
                    0.5277916193008423, 0.028420301154255867,
                    -0.6749999523162842, 2, 0.3484765887260437,
                    0.4734894633293152, 0.3897113800048828, 0,
                ]);
                const transforms =
            ...
            return loadGltf(instancedWithNormalizedRotation).then(
                function (gltfLoader) {
                    ...
                    const secondMatrixComponents = [
                        1.1007905724243354, 0.07140440309598281,
                        -0.1331359457080602, 0, -0.04344372372420601,
                        1.0874251248973055, 0.22401538735190446, 0,
                        0.1446942006095891, -0.21672946758564085,
                        1.0801183172918447, 0, 1.1111111640930176,
                        1.1111111640930176, 1.1111111640930176, 1,
                    ];
            ...
            return loadGltf(boxInstancedTranslationMinMax).then(
                function (gltfLoader) {
                    ...
                    ShaderBuilderTester.expectHasFragmentDefines(
                        shaderBuilder,
                        ["HAS_INSTANCING", "HAS_INSTANCE_TRANSLATION"],
                    );
                    ...
                    expect(
                        renderResources.model._pipelineResources.length,
                    ).toEqual(0);
                    expect(
                        renderResources.model._modelResources.length,
                    ).toEqual(0);
        ...
        it("adds uniforms for legacy instancing path", function () {
            ...
            return loadI3dm(i3dmInstancedOrientation, {
                ...
                const uniformMap = renderResources.uniformMap;
                expect(
                    uniformMap.u_instance_modifiedModelView(),
                ).toEqualEpsilon(expectedModelView, CesiumMath.EPSILON8);
