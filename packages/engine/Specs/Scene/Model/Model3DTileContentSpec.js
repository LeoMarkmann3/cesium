...
describe(
    ...
    function () {
        ...
        const glbContentUrl =
            "./Data/Cesium3DTiles/GltfContent/glb/tileset.json";
        ...
        function setCamera(longitude, latitude, range) {
            ...
            scene.camera.lookAt(
                center,
                new HeadingPitchRange(0.0, -1.57, range),
            );
        }
        ...
        describe("b3dm", function () {
            ...
            it("picks from b3dm", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(scene).toPickAndCall(function (result) {
                        ...
                        expect(content.hasProperty(featureId, "id")).toBe(
                            false,
                        );
            ...
            it("gets memory usage", function () {
                return Cesium3DTilesTester.loadTileset(scene, texturedUrl).then(
                    function (tileset) {
                        ...
                        const batchTexturesByteLength =
                            content.featuresLength * 4;
                        const pickTexturesByteLength =
                            content.featuresLength * 4;
                        ...
                        expect(content.texturesByteLength).toEqual(
                            texturesByteLength,
                        );
            ...
            it("toggles showing copyright from glTF on screen", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(lightboxCredits[0].credit.html).toEqual(
                        "Sample Copyright",
                    );
                    ...
                    expect(screenCredits[0].credit.html).toEqual(
                        "Sample Copyright",
                    );
            ...
            it("picks from i3dm batch table", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    expect(scene).toPickAndCall(function (result) {
                        ...
                        expect(content.hasProperty(featureId, "Height")).toBe(
                            true,
                        );
                        expect(content.getFeature(featureId)).toBeDefined();
                    });
        ...
        describe("point cloud attenuation", function () {
            ...
            function attenuationTest(postLoadCallback) {
                ...
                scene.msaaSamples = 1;
                noAttenuationPixelCount = scene.logarithmicDepthBuffer
                    ? 20
                    : 16;
                ...
                scene.camera.lookAt(
                    center,
                    new HeadingPitchRange(0.0, -1.57, 5.0),
                );
            ...
            it("attenuates points based on geometric error", function () {
                return attenuationTest(function (scene, tileset) {
                    ...
                    expect(scene).toRenderPixelCountAndCall(
                        function (pixelCount) {
                            expect(pixelCount).toBeGreaterThan(
        ...
        describe("clipping planes", function () {
            ...
            it("Links model to tileset clipping planes if tileset clipping planes are reassigned", function () {
                return Cesium3DTilesTester.loadTileset(
                    ...
                    const newClippingPlaneCollection =
                    ...
                    expect(model.clippingPlanes).not.toBe(
        ...
        describe("classification", function () {
            ...
            beforeEach(function () {
                ...
                globePrimitive = new MockPrimitive(
                    reusableGlobePrimitive,
                    Pass.GLOBE,
                );
