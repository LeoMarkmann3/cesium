    ...
    function () {
        ...
        it("counts memory for a model", function () {
            return loadGltf(boxTextured).then(function (gltfLoader) {
                ...
                PrimitiveStatisticsPipelineStage.process(
                    renderResources,
                    primitive,
                );
                ...
                expect(statistics.geometryByteLength).toBe(
                    expectedGeometryLength,
                );
                expect(statistics.texturesByteLength).toBe(
                    expectedTexturesLength,
                );
        ...
        it("_countGeometry computes memory usage for a triangle mesh", function () {
            return loadGltf(boxTextured).then(function (gltfLoader) {
                ...
                PrimitiveStatisticsPipelineStage._countGeometry(
                    statistics,
                    primitive,
                );
        ...
        it("_countGeometry computes memory usage for CESIUM_primitive_outline", function () {
            return loadGltf(boxWithPrimitiveOutline).then(
                function (gltfLoader) {
                    ...
                    expectedLength +=
                        primitive.outlineCoordinates.buffer.sizeInBytes;
        ...
        it("_countBinaryMetadata updates statistics for propertyTextures", function () {
            return loadGltf(simplePropertyTexture).then(function (gltfLoader) {
                ...
                // once.
                const structuralMetadata = model.structuralMetadata;
                const propertyTexture1 =
                    structuralMetadata.getPropertyTexture(0);
                const property =
                    propertyTexture1.getProperty("insideTemperature");
