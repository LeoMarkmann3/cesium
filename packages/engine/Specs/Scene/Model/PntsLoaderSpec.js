...
describe(
    ...
    function () {
        ...
        async function expectLoadError(arrayBuffer) {
            await expectAsync(
                loadPntsArrayBuffer(arrayBuffer),
        ...
        function expectMetadata(
            ...
            const batchClass =
                schema.classes[MetadataClass.BATCH_TABLE_CLASS_NAME];
            ...
            for (const propertyName in expectedProperties) {
                if (expectedProperties.hasOwnProperty(propertyName)) {
                    ...
                    if (expectedProperty.isJson) {
                        ...
                        expect(
                            propertyTable.getProperty(0, propertyName),
                        ).toBeDefined();
            ...
            if (isBatched) {
                ...
                expect(
                    Object.keys(propertyAttribute.properties).sort(),
                ).toEqual(attributePropertyNames.sort());
            }
        }
        ...
        function expectColorRGB(attribute) {
            ...
            expect(attribute.componentDatatype).toBe(
        ...
        it("loads PointCloudNormalsOctEncoded", function () {
            return loadPnts(pointCloudNormalsOctEncodedUrl).then(
                function (loader) {
                    const components = loader.components;
                    expect(components).toBeDefined();
                    expectEmptyMetadata(components.structuralMetadata);
        ...
        it("loads PointCloudQuantizedOctEncoded", function () {
            return loadPnts(pointCloudQuantizedOctEncodedUrl).then(
                function (loader) {
                    const components = loader.components;
                    expect(components).toBeDefined();
                    expectEmptyMetadata(components.structuralMetadata);
        ...
        it("BATCH_ID semantic uses componentType of UNSIGNED_SHORT by default", function () {
            const arrayBuffer =
                Cesium3DTilesTester.generatePointCloudTileBuffer({
                    featureTableJson: {
                        POINTS_LENGTH: 2,
        ...
        it("loads attributes for 2D", function () {
            return loadPnts(pointCloudRGBUrl, {
