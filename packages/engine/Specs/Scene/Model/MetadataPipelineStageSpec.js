    ...
    function () {
        ...
        it("Adds property attributes to the shader", function () {
            return loadGltf(pointCloudWithPropertyAttributes).then(
                function (gltfLoader) {
                    ...
                    expect(uniformMap.u_toroidalAngle_scale()).toBe(
                        0.21666156231653746,
                    );
                    expect(uniformMap.u_poloidalAngle_offset()).toBe(
                        -3.141592653589793,
                    );
                    expect(uniformMap.u_poloidalAngle_scale()).toBe(
                        0.3306939635357677,
                    );
                },
            );
            ...
            return loadGltf(propertyTextureWithVectorProperties).then(
                function (gltfLoader) {
                    ...
                    const propertyTexture1 =
                        structuralMetadata.getPropertyTexture(0);
                    const texture1 =
                        propertyTexture1.getProperty("arrayProperty");
                    ...
                    expect(
                        uniformMap.u_valueTransformProperty_offset(),
                    ).toEqual(new Cartesian2(1, 1));
        ...
        it("Handles a tileset with metadata statistics", function () {
            ...
            return Cesium3DTilesTester.loadTileset(
                ...
                const primitive =
                    model.sceneGraph.components.nodes[0].primitives[0];
