...
describe(
    ...
    function () {
        ...
        function verifyDerivedCommandsDefined(drawCommand, expected) {
            ...
            const skipLevelOfDetailDefined =
                expected.skipLevelOfDetail ?? false;
            ...
            expect(defined(skipLodBackfaceCommand)).toBe(
                skipLevelOfDetailDefined,
            );
            expect(defined(skipLodStencilCommand)).toBe(
                skipLevelOfDetailDefined,
            );
        ...
        function verifyDerivedCommandUpdateFlags(derivedCommand, expected) {
            expect(derivedCommand.updateShadows).toEqual(
                expected.updateShadows,
            );
            expect(derivedCommand.updateBackFaceCulling).toEqual(
                expected.updateBackFaceCulling,
            );
            expect(derivedCommand.updateCullFace).toEqual(
                expected.updateCullFace,
            );
        ...
        describe("skipLeveOfDetail commands", function () {
            ...
            function verifySkipLodBackfaceCommand(command) {
                ...
                expect(renderState.colorMask).toEqual(expectedColorMask);
                expect(renderState.polygonOffset).toEqual(
                    expectedPolygonOffset,
                );

                const uniformMap = command.uniformMap;
                expect(uniformMap.u_polygonOffset()).toEqual(
            ...
            function verifySkipLodStencilCommand(command) {
                ...
                expect(stencilTest.mask).toEqual(
                    StencilConstants.SKIP_LOD_MASK,
                );
            ...
            it("constructs skipLevelOfDetail commands", function () {
                ...
                const skipLodBackfaceCommand =
                    drawCommand._skipLodBackfaceCommand;
                ...
                const skipLodStencilCommand =
                    drawCommand._skipLodStencilCommand;
        ...
        describe("pushCommands", function () {
            ...
            it("pushCommands pushes translucent command if style is ALL_TRANSLUCENT", function () {
                ...
                const translucentDrawCommand =
                    drawCommand._translucentCommand.command;
            ...
            it("pushCommands pushes both commands if style is OPAQUE_AND_TRANSLUCENT", function () {
                ...
                const model = renderResources.model;
                model.styleCommandsNeeded =
                    StyleCommandsNeeded.OPAQUE_AND_TRANSLUCENT;

                const originalDrawCommand =
                    drawCommand._originalCommand.command;
                const translucentDrawCommand =
                    drawCommand._translucentCommand.command;
            ...
            it("pushCommands pushes skipLevelOfDetail commands", function () {
                ...
                expect(backfaceCommands[0]).toBe(
                    skipLodBackfaceCommand.command,
                );
            });
            ...
            it("pushCommands derives 2D command if model is near IDL", function () {
                ...
                expect(originalDrawCommand.modelMatrix).toBe(
            ...
            it("pushCommands derives 2D translucent command", function () {
                ...
                const translucentCommand2D =
                    translucentCommand.derivedCommand2D;
            ...
            it("pushCommands derives 2D silhouette commands", function () {
                ...
                // 2D commands aren't derived until pushCommands is called
                const silhouetteModelCommand =
                    drawCommand._silhouetteModelCommand;
                const silhouetteColorCommand =
                    drawCommand._silhouetteColorCommand;
                ...
                expect(modelDrawCommand.modelMatrix).toBe(
                    drawCommand._modelMatrix,
                );
                expect(modelDrawCommand2D.modelMatrix).toBe(
                    drawCommand._modelMatrix2D,
                );

                const colorDrawCommand = silhouetteColorCommand.command;
                const colorDrawCommand2D = silhouetteColorCommand2D.command;
                expect(colorDrawCommand.modelMatrix).toBe(
                    drawCommand._modelMatrix,
                );
                expect(colorDrawCommand2D.modelMatrix).toBe(
                    drawCommand._modelMatrix2D,
                );
            ...
            it("pushCommands derives 2D skipLevelOfDetail commands", function () {
                ...
                const skipLodStencilCommand2D =
                    skipLodStencilCommand.derivedCommand2D;
                ...
                expect(backfaceDrawCommand.modelMatrix).toBe(
                    drawCommand._modelMatrix,
                );
                ...
                expect(stencilDrawCommand.modelMatrix).toBe(
                    drawCommand._modelMatrix,
                );
            ...
            it("pushCommands updates model matrix for 2D commands", function () {
                ...
                expect(originalDrawCommand.boundingVolume.center).toEqual(
                    translation,
                );
                ...
                expect(originalDrawCommand2D.modelMatrix).toEqual(
                    expectedModelMatrix,
                );
        ...
        describe("model matrix", function () {
            it("updates model matrix", function () {
                ...
                expect(drawCommand.boundingVolume.center).toEqual(
                    Cartesian3.ZERO,
                );
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    expect(command.boundingVolume.center).toEqual(
                        Cartesian3.ZERO,
                    );
                }
        ...
        describe("back face culling", function () {
            it("updates back face culling for opaque command", function () {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    const updateBackFaceCulling =
                        derivedCommand.updateBackFaceCulling;
                    if (!updateBackFaceCulling) {
                        expect(derivedCommand).toBe(
                            drawCommand._translucentCommand,
                        );
                    }

                    expect(command.renderState.cull.enabled).toBe(
            ...
            it("doesn't update back face culling for skipLevelOfDetail backface command", function () {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    if (isTranslucentCommand) {
                        expect(derivedCommand.updateBackFaceCulling).toBe(
                            false,
                        );
                        expect(command.renderState.cull.enabled).toBe(false);
            ...
            it("updates cull face for opaque command", function () {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    if (!updateCullFace) {
                        ...
                        expect(command.renderState.cull.face).toBe(
                            CullFace.BACK,
                        );
                    } else {
                        expect(command.renderState.cull.face).toBe(
