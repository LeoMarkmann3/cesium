...
describe("Scene/GlobeSurfaceTile", function () {
    ...
    describe("processStateMachine", function () {
        ...
        it("transitions to the LOADING state immediately if this tile is available", function () {
            ...
            return processor
                .process([rootTile.southwestChild])
                .then(function () {
        ...
        it("pushes parent along if waiting on it to be able to upsample", function () {
            ...
            return processor
                .process([rootTile.southwestChild])
                .then(function () {
                    expect(mockTerrain.requestTileGeometry.calls.count()).toBe(
                        1,
                    );
                    expect(
                        mockTerrain.requestTileGeometry.calls.argsFor(0)[0],
                    ).toBe(0);
                    expect(
                        mockTerrain.requestTileGeometry.calls.argsFor(0)[1],
                    ).toBe(0);
                    expect(
                        mockTerrain.requestTileGeometry.calls.argsFor(0)[2],
                    ).toBe(0);
                });
        });
        ...
        it("prints error message when a root tile fails to load", function () {
            ...
            return processor.process([rootTile]).then(function () {
                ...
                expect(
                    TileProviderError.reportError.calls.argsFor(0)[3],
                ).toContain(
        ...
        it("marks an upsampled tile as such", function () {
            ...
            return processor
                ...
                .then(function () {
                    ...
                    expect(rootTile.southwestChild.upsampledFromParent).toBe(
                        true,
                    );
                });
        });
