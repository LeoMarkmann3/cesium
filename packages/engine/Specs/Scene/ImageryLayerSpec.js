...
describe(
    ...
    function () {
        ...
        it("readyEvent is raised when asynchronous provider become ready", async function () {
            ...
            expect(layer.ready).toBeTrue();
            expect(layer.imageryProvider).toBeInstanceOf(
                SingleTileImageryProvider,
            );
        });
        ...
        async function createWebMercatorProvider() {
            ...
            return BingMapsImageryProvider.fromUrl("http://host.invalid", {
                key: "",
                tileDiscardPolicy: new NeverTileDiscardPolicy(),
            });
        }

        it("reprojects web mercator images when necessary", async function () {
            ...
            return pollToPromise(function () {
                ...
                return pollToPromise(function () {
                    ...
                    const textureBeforeReprojection =
                    ...
                    return pollToPromise(function () {
                        ...
                        expect(
                            imagery.texture.sampler.minificationFilter,
                        ).toEqual(
                            TextureMinificationFilter.LINEAR_MIPMAP_LINEAR,
                        );
                        expect(
                            imagery.texture.sampler.magnificationFilter,
                        ).toEqual(TextureMinificationFilter.LINEAR);
                        expect(textureBeforeReprojection).not.toEqual(
                            imagery.texture,
                        );
                        imagery.releaseReference();
                    });
        ...
        it("does not reproject web mercator images when not necessary", async function () {
            const provider = await createWebMercatorProvider();
            const layer = new ImageryLayer(provider);
        ...
        it("reprojects web mercator images later if it becomes necessary later", async function () {
            const provider = await createWebMercatorProvider();
            const layer = new ImageryLayer(provider);
        ...
        it("cancels reprojection", async function () {
            const provider = await createWebMercatorProvider();
            const layer = new ImageryLayer(provider);
        ...
        it("getImageryRectangle works", async function () {
            const providerRectangle = Rectangle.fromDegrees(
                8.2,
                61.09,
                8.5,
                61.7,
        ...
        describe("createTileImagerySkeletons", function () {
            it("handles a base layer that does not cover the entire globe", async function () {
                ...
                expect(
                    tiles[1].data.imagery[0].textureCoordinateRectangle.x,
                ).toBe(0.0);
                expect(
                    tiles[1].data.imagery[0].textureCoordinateRectangle.w,
                ).toBe(1.0);
                expect(
                    tiles[1].data.imagery[0].textureCoordinateRectangle.z,
                ).toBe(1.0);
                expect(
                    tiles[1].data.imagery[1].textureCoordinateRectangle.x,
                ).toBe(0.0);
                expect(
                    tiles[1].data.imagery[1].textureCoordinateRectangle.y,
                ).toBe(0.0);
                expect(
                    tiles[1].data.imagery[1].textureCoordinateRectangle.z,
                ).toBe(1.0);
            });

            it("does not get confused when base layer imagery overlaps in one direction but not the other", async function () {
                ...
                const wholeWorldProvider =
                    await SingleTileImageryProvider.fromUrl(
                ...
                const layers = new ImageryLayerCollection();
                const wholeWorldLayer =
                    layers.addImageryProvider(wholeWorldProvider);
                const terrainProvider = new EllipsoidTerrainProvider();
