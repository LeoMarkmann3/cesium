...
describe(
    ...
    function () {
        ...
        it("default constructs", function () {
            ...
            expect(groundPolylinePrimitive.releaseGeometryInstances).toEqual(
                true,
            );
            expect(groundPolylinePrimitive.allowPicking).toEqual(true);
            expect(groundPolylinePrimitive.asynchronous).toEqual(true);
            expect(groundPolylinePrimitive.debugShowBoundingVolume).toEqual(
                false,
            );
            expect(groundPolylinePrimitive.debugShowShadowVolume).toEqual(
                false,
            );
        });
        ...
        it("renders during morph when scene3DOnly is false", function () {
            ...
            expect(scene).toRenderAndCall(function (rgba) {
                expect(
                    coordinateOfPixelColor(rgba, polylineColor),
                ).toBeDefined();
            });
        ...
        xit("renders with distance display condition per instance attribute", function () {
            ...
            expect(scene).toRenderAndCall(function (rgba) {
                expect(
                    coordinateOfPixelColor(rgba, depthColor),
                ).toBeUndefined();
            });
        ...
        it("picking in Morph", function () {
            ...
            expect(scene).toRenderAndCall(function (rgba) {
                polylineColorCoordinate = coordinateOfPixelColor(
                    rgba,
                    polylineColor,
                );
            });
