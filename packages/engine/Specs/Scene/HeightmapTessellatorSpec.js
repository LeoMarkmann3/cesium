...
describe("Scene/HeightmapTessellator", function () {
        ...
        for (let j = 0; j < height; ++j) {
            ...
            for (let i = 0; i < width; ++i) {
                ...
                const expectedU =
                    ...
                    (geographicNortheast.longitude -
                        geographicSouthwest.longitude);
                const expectedV =
                    (latitude - geographicSouthwest.latitude) /
                    (geographicNortheast.latitude -
                        geographicSouthwest.latitude);

                expect(vertexPosition).toEqualEpsilon(
                    expectedVertexPosition,
                    1.0,
                );
                expect(vertices[index + 3]).toEqual(heightSample);
    ...
    it("supports multi-element little endian heights", function () {
        ...
        const options = {
            heightmap: [
                1.0, 2.0, 100.0, 3.0, 4.0, 100.0, 5.0, 6.0, 100.0, 7.0, 8.0,
                100.0, 9.0, 10.0, 100.0, 11.0, 12.0, 100.0, 13.0, 14.0, 100.0,
                15.0, 16.0, 100.0, 17.0, 18.0, 100.0,
            ],
            width: width,
            height: height,
            skirtHeight: 0.0,
        ...
        for (let j = 0; j < height; ++j) {
            ...
            for (let i = 0; i < width; ++i) {
                ...
                const heightSampleIndex =
                    (j * width + i) * options.structure.stride;
                const heightSample =
