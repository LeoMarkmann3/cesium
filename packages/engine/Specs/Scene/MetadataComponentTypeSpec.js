...
describe("Scene/MetadataComponentType", function () {
    it("getMinimum", function () {
        expect(
            MetadataComponentType.getMinimum(MetadataComponentType.INT8),
        ).toBe(-128);
        expect(
            MetadataComponentType.getMinimum(MetadataComponentType.UINT8),
        ).toBe(0);
        expect(
            MetadataComponentType.getMinimum(MetadataComponentType.INT16),
        ).toBe(-32768);
        expect(
            MetadataComponentType.getMinimum(MetadataComponentType.UINT16),
        ).toBe(0);
        expect(
            MetadataComponentType.getMinimum(MetadataComponentType.INT32),
        ).toBe(-2147483648);
        expect(
            MetadataComponentType.getMinimum(MetadataComponentType.UINT32),
        ).toBe(0);
        expect(
    ...
    it("getMaximum", function () {
        expect(
            MetadataComponentType.getMaximum(MetadataComponentType.INT8),
        ).toBe(127);
        expect(
            MetadataComponentType.getMaximum(MetadataComponentType.UINT8),
        ).toBe(255);
        expect(
            MetadataComponentType.getMaximum(MetadataComponentType.INT16),
        ).toBe(32767);
        expect(
            MetadataComponentType.getMaximum(MetadataComponentType.UINT16),
        ).toBe(65535);
        expect(
            MetadataComponentType.getMaximum(MetadataComponentType.INT32),
        ).toBe(2147483647);
        expect(
            MetadataComponentType.getMaximum(MetadataComponentType.UINT32),
        ).toBe(4294967295);
        expect(
    ...
    it("normalizes signed integers", function () {
        ...
        const middle = [
            0.5039370078740157, 0.500015259254738, 0.5000000002328306,
        ];

        for (let i = 0; i < signedTypes.length; ++i) {
            const type = signedTypes[i];
            const min = MetadataComponentType.getMinimum(
                MetadataComponentType[type],
            );
            const max = MetadataComponentType.getMaximum(
                MetadataComponentType[type],
            );
