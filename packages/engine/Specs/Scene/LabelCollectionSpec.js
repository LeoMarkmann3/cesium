...
describe("Scene/LabelCollection", function () {
    ...
    it("can add a label with specified values", function () {
        ...
        expect(label.distanceDisplayCondition).toEqual(
            distanceDisplayCondition,
        );
        expect(label.disableDepthTestDistance).toEqual(
    ...
    describe(
        ...
        function () {
            ...
            it("picks a label using translucencyByDistance", function () {
                ...
                return allLabelsReady().then(function () {
                    const translucency = new NearFarScalar(
                        1.0,
                        0.9,
                        3.0e9,
                        0.8,
            ...
            it("picks a label using pixelOffsetScaleByDistance", function () {
                ...
                return allLabelsReady().then(function () {
                    const pixelOffsetScale = new NearFarScalar(
                        1.0,
                        0.0,
                        3.0e9,
                        0.0,
            ...
            it("should reuse canvases for letters, but only if other settings are the same", async function () {
                ...
                const cache =
                    labels._glyphBillboardCollection.billboardTextureCache;
            ...
            describe("Label", function () {
                it("should update background billboard when updating label properties while label is hidden", async function () {
                    ...
                    await allLabelsReady();
                    expect(
                        labels._backgroundBillboardCollection.length,
                    ).toEqual(1);
                ...
                it("can compute screen space position", function () {
                    ...
                    scene.renderForSpecs();
                    expect(
                        label.computeScreenSpacePosition(scene),
                    ).toEqualEpsilon(
                ...
                it("stores screen space position in a result", function () {
                    ...
                    scene.renderForSpecs();
                    const actual = label.computeScreenSpacePosition(
                        scene,
                        result,
                    );
                ...
                it("computes screen space bounding box with horizontal origin", function () {
                    ...
                    return allLabelsReady().then(function () {
                        let bbox = Label.getScreenSpaceBoundingBox(
                            label,
                            Cartesian2.ZERO,
                        );
                        ...
                        scene.renderForSpecs();
                        bbox = Label.getScreenSpaceBoundingBox(
                            label,
                            Cartesian2.ZERO,
                        );
                ...
                it("sets billboard properties properly when they change on the label", function () {
                    ...
                    const id1 = "id1";
                    const id2 = "id2";
                    const translucency1 = new NearFarScalar(
                        1.0e4,
                        1.0,
                        1.0e6,
                        0.0,
                    );
                    const translucency2 = new NearFarScalar(
                        1.1e4,
                        1.2,
                        1.3e6,
                        4.0,
                    );
                    const pixelOffsetScale1 = new NearFarScalar(
                        1.0e4,
                        1.0,
                        1.0e6,
                        0.0,
                    );
                    const pixelOffsetScale2 = new NearFarScalar(
                        1.5e4,
                        1.6,
                        1.7e6,
                        8.0,
                    );
                    const scaleByDistance1 = new NearFarScalar(
                        1.0e4,
                        1.0,
                        1.0e6,
                        0.0,
                    );
                    const scaleByDistance2 = new NearFarScalar(
                        1.5e4,
                        1.6,
                        1.7e6,
                        8.0,
                    ...
                    for (let i = 0; i < label._glyphs.length; ++i) {
                        ...
                        expect(billboard.pixelOffset).toEqual(
                            label.pixelOffset,
                        );
                        expect(billboard.verticalOrigin).toEqual(
                            label.verticalOrigin,
                        );
                        // glyph horizontal origin is always LEFT
                        expect(billboard.scale).toEqual(
                            label.scale * label._relativeSize,
                        );
                ...
                describe("sets individual billboard properties properly when they change on the label", function () {
                    ...
                    it("eyeOffset", function () {
                        ...
                        getGlyphBillboards().forEach(function (billboard) {
                            expect(billboard.eyeOffset).toEqual(
                                label.eyeOffset,
                            );
                        });
                    });
                    ...
                    it("translucencyByDistance", function () {
                        const newValue = new NearFarScalar(
                            1.1e4,
                            1.2,
                            1.3e6,
                            4.0,
                        );
                        expect(label.translucencyByDistance).not.toEqual(
                            newValue,
                        );
                    ...
                    it("pixelOffsetScaleByDistance", function () {
                        ...
                        expect(label.pixelOffsetScaleByDistance).not.toEqual(
                            newValue,
                        );
                        ...
                        getGlyphBillboards().forEach(function (billboard) {
                            ...
                            ).toEqual(label.pixelOffsetScaleByDistance);
                        });
                    });
                ...
                it("should set vertexTranslate of billboards correctly when vertical origin is changed", function () {
                    ...
                    return allLabelsReady().then(function () {
                        ...
                        const offset0 = getGlyphBillboardVertexTranslate(
                            label,
                            0,
                        );
                        const offset1 = getGlyphBillboardVertexTranslate(
                            label,
                            1,
                        );
                        const offset2 = getGlyphBillboardVertexTranslate(
                            label,
                            2,
                        );
                ...
                it("should set vertexTranslate of billboards correctly when horizontal origin is changed", function () {
                    ...
                    return allLabelsReady().then(function () {
                        ...
                        const offsetBack =
                            getBackgroundBillboardVertexTranslate(label);
                        ...
                        expect(
                            getGlyphBillboardVertexTranslate(label, 0).y,
                        ).toEqual(offset0.y);
                        expect(
                            getGlyphBillboardVertexTranslate(label, 1).y,
                        ).toEqual(offset1.y);
                        expect(
                            getGlyphBillboardVertexTranslate(label, 2).y,
                        ).toEqual(offset2.y);
                        expect(
                            getBackgroundBillboardVertexTranslate(label).y,
                        ).toEqual(offsetBack.y);
                        ...
                        expect(
                            getGlyphBillboardVertexTranslate(label, 0).x,
                        ).toBeLessThan(offset0.x);
                        expect(
                            getGlyphBillboardVertexTranslate(label, 1).x,
                        ).toBeLessThan(offset1.x);
                        expect(
                            getGlyphBillboardVertexTranslate(label, 2).x,
                        ).toBeLessThan(offset2.x);
                        expect(
                            getBackgroundBillboardVertexTranslate(label).x,
                        ).toBeLessThan(offsetBack.x);
                ...
                it("should set vertexTranslate of billboards correctly when scale is changed", function () {
                    ...
                    return allLabelsReady().then(function () {
                        ...
                        let offset0 = getGlyphBillboardVertexTranslate(
                            label,
                            0,
                        );
                        let offset1 = getGlyphBillboardVertexTranslate(
                            label,
                            1,
                        );
                        let offset2 = getGlyphBillboardVertexTranslate(
                            label,
                            2,
                        );
                ...
                it("label vertex translate should remain the same when pixel offset is changed", function () {
                    ...
                    return allLabelsReady().then(function () {
                        ...
                        expect(
                            getGlyphBillboardVertexTranslate(label, 0),
                        ).toEqual(offset0);
                        expect(
                            getGlyphBillboardVertexTranslate(label, 1),
                        ).toEqual(offset1);
                        expect(
                            getGlyphBillboardVertexTranslate(label, 2),
                        ).toEqual(offset2);
                ...
                it("Correctly updates billboard position when height reference changes", async function () {
                    ...
                    const labelsWithScene = new LabelCollection({
                        scene: scene,
                    });
                ...
                it("should have the same vertexTranslate of billboards whether values are set at construction or afterwards", function () {
                    ...
                    return allLabelsReady().then(function () {
                        expect(
                            getGlyphBillboardVertexTranslate(one, 0),
                        ).toEqual(getGlyphBillboardVertexTranslate(two, 0));
                        expect(
                            getGlyphBillboardVertexTranslate(one, 1),
                        ).toEqual(getGlyphBillboardVertexTranslate(two, 1));
                        expect(
                            getGlyphBillboardVertexTranslate(one, 2),
                        ).toEqual(getGlyphBillboardVertexTranslate(two, 2));
                    });
                });
                ...
                it("should not change label dimensions when scale changes", function () {
                    ...
                    return allLabelsReady().then(function () {
                        ...
                        const dimensions = label._glyphs[0].dimensions;
                        expect(dimensions.width).toEqual(
                            originalDimensions.width,
                        );
                        expect(dimensions.height).toEqual(
                            originalDimensions.height,
                        );
                        expect(dimensions.descent).toEqual(
                            originalDimensions.descent,
                        );
                    });
                });
                ...
                it("should increase label height and decrease width when adding newlines", function () {
                    ...
                    return allLabelsReady().then(function () {
                        ...
                        expect(newlinesBbox.width).toBeLessThan(
                            originalBbox.width,
                        );
                        expect(newlinesBbox.height).toBeGreaterThan(
                            originalBbox.height,
                        );
                    });
                });
            ...
            it("computes bounding sphere in 3D", function () {
                ...
                return allLabelsReady().then(function () {
                    const actual =
                        scene.frameState.commandList[0].boundingVolume;
            ...
            it("computes bounding sphere in Columbus view", function () {
                ...
                return allLabelsReady().then(function () {
                    ...
                    const projectedPositions = [
                        projection.project(
                            ellipsoid.cartesianToCartographic(one.position),
                        ),
                        projection.project(
                            ellipsoid.cartesianToCartographic(two.position),
                        ),
                    ];
                    const expected =
                        BoundingSphere.fromPoints(projectedPositions);
                    ...
                    expect(actual.radius).toBeGreaterThanOrEqual(
                        expected.radius,
                    );
                    scene.screenSpaceCameraController.enableCollisionDetection =
            ...
            it("destroys texture atlas when destroying", async function () {
                ...
                const background =
                    labels._backgroundBillboardCollection.textureAtlas;
            ...
            describe("height referenced labels", function () {
                ...
                it("explicitly constructs a label with height reference", function () {
                    ...
                    expect(l.heightReference).toEqual(
                ...
                it("removes the callback when the height reference changes", function () {
                    ...
                    spyOn(scene, "updateHeight").and.returnValue(
                        removeCallback,
                    );
                ...
                it("resets the clamped position when HeightReference.NONE", async function () {
                    ...
                    expect(
                        l._glyphs[0].billboard._clampedPosition,
                    ).toBeUndefined();
                });
                ...
                it("clears the billboard height reference callback when the label is removed", async function () {
                    ...
                    expect(
                        labelsWithHeight._spareBillboards[0]
                            ._removeCallbackFunc,
                    ).toBeUndefined();
                });
            });
