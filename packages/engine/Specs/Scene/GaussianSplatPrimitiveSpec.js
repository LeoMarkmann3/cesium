...
describe(
    ...
    function () {
        ...
        it("Check Spherical Harmonic specular on a Gaussian splats tileset", async function () {
            ...
            await Cesium3DTilesTester.waitForTileContentReady(
                scene,
                tileset.root,
            );
            ...
            scene.renderForSpecs();
            expect(scene).toRenderAndCall(function (rgba) {
                expect(rgba[samplePosition + 0]).toBeCloseTo(
                    targetYellow.red,
                    -1,
                );
                expect(rgba[samplePosition + 1]).toBeCloseTo(
                    targetYellow.green,
                    -1,
                );
                expect(rgba[samplePosition + 2]).toBeCloseTo(
                    targetYellow.blue,
                    -1,
                );
            });
