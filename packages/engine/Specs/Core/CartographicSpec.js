...
describe("Core/Cartographic", function () {
    ...
    it("toCartesian conversion from Cartographic input to Cartesian3 output", function () {
        ...
        const actual = Cartographic.toCartesian(
    ...
    it("fromRadians works with a result parameter", function () {
        ...
        const c = Cartographic.fromRadians(
            Math.PI / 2,
            Math.PI / 4,
            100.0,
            result,
    ...
    it("fromCartesian uses default ellipsoid", function () {
        ...
        const expectedCartographic = Cartographic.fromDegrees(
            23.47315,
            0.67416,
        );
    ...
    it("fromCartesian works with a value that is above the ellipsoid surface", function () {
        ...
        expect(cartographic2).toEqualEpsilon(
            cartographic1,
            CesiumMath.EPSILON8,
        );
    });
    ...
    it("equals", function () {
        const cartographic = new Cartographic(1.0, 2.0, 3.0);
        expect(cartographic.equals(new Cartographic(1.0, 2.0, 3.0))).toEqual(
            true,
        );
        expect(cartographic.equals(new Cartographic(2.0, 2.0, 3.0))).toEqual(
            false,
        );
        expect(cartographic.equals(new Cartographic(2.0, 1.0, 3.0))).toEqual(
            false,
        );
        expect(cartographic.equals(new Cartographic(1.0, 2.0, 4.0))).toEqual(
            false,
        );
    ...
    it("equalsEpsilon", function () {
        ...
        expect(
            cartographic.equalsEpsilon(
                new Cartographic(2.0, 2.0, 3.0),
                0.99999,
            ),
        ).toEqual(false);
        expect(
            cartographic.equalsEpsilon(
                new Cartographic(1.0, 3.0, 3.0),
                0.99999,
            ),
        ).toEqual(false);
        expect(
            cartographic.equalsEpsilon(
                new Cartographic(1.0, 2.0, 4.0),
                0.99999,
            ),
        ).toEqual(false);
