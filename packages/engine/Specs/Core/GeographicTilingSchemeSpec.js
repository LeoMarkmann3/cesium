...
describe("Core/GeographicTilingScheme", function () {
    ...
    describe("positionToTileXY", function () {
        it("returns undefined when outside rectangle", function () {
            ...
            expect(
                tilingScheme.positionToTileXY(tooFarWest, 0),
            ).toBeUndefined();
            const tooFarSouth = new Cartographic(0.2, 0.1);
            expect(
                tilingScheme.positionToTileXY(tooFarSouth, 0),
            ).toBeUndefined();
            const tooFarEast = new Cartographic(0.4, 0.3);
            expect(
                tilingScheme.positionToTileXY(tooFarEast, 0),
            ).toBeUndefined();
            const tooFarNorth = new Cartographic(0.2, 0.5);
            expect(
                tilingScheme.positionToTileXY(tooFarNorth, 0),
            ).toBeUndefined();
        });
        ...
        it("returns correct tile for position in center of tile", function () {
            ...
            const centerOfWesternRootTile = new Cartographic(
                -Math.PI / 2.0,
                0.0,
            );
            expect(
                tilingScheme.positionToTileXY(centerOfWesternRootTile, 0),
            ).toEqual(new Cartesian2(0, 0));
