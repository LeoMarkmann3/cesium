...
describe("Core/EarthOrientationParameters", function () {
    ...
    describe("loading eop", function () {
            ...
            const dt =
                JulianDate.secondsDifference(
                    ...
                    JulianDate.fromIso8601(
                        eopDescription.data.samples[nColumns],
                    ),
                ) / 86400.0;
        ...
        it("interpolates UT1 correctly over a leap second", function () {
            ...
            const eop = new EarthOrientationParameters(eopDescription);
            const dateAtLeapSecond = JulianDate.fromIso8601(
                "2009-01-01T00:00:00Z",
            );
            ...
            const t0 = JulianDate.fromIso8601(
                eopDescription.data.samples[nColumns],
            );
            ...
            let dt =
                JulianDate.secondsDifference(dateSlightlyBefore, t0) /
                (86400.0 + 1);
            ...
            expect(resultBefore.ut1MinusUtc).toEqualEpsilon(
                expectedBefore,
                1.0e-15,
            );
            ...
            expect(resultAfter.ut1MinusUtc).toEqualEpsilon(
                expectedAfter,
                1.0e-15,
            );
            // Check to make sure the values are (correctly) discontinuous
            expect(
                Math.abs(resultBefore.ut1MinusUtc - resultAfter.ut1MinusUtc) >
                    0.5,
            ).toEqual(true);
        });
    });
