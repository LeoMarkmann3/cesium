...
describe("Core/HermitePolynomialApproximation", function () {
    ...
    const yTable = [
        13378137.0, 0, 13374128.3576279, 0, 13362104.8328212, 0,
        13342073.6310691, 0, 13314046.7567223, 0, 13278041.005799, 0,
        13234077.9559193, 0, 13182183.953374, 0,
    ];
    const dyTable = [
        0.0, 0, -133.614738921601, 0, -267.149404854867, 0, -400.523972797808,
        0, -533.658513692378, 0, -666.473242324565, 0, -798.888565138278, 0,
        -930.82512793439, 0,
    ];
    ...
    it("getRequiredDataPoints should be 1 more than degree, except for 0, which requires 2", function () {
        expect(HermitePolynomialApproximation.getRequiredDataPoints(0)).toEqual(
            2,
        );
        expect(HermitePolynomialApproximation.getRequiredDataPoints(1)).toEqual(
            2,
        );
        expect(HermitePolynomialApproximation.getRequiredDataPoints(2)).toEqual(
            3,
        );
        expect(HermitePolynomialApproximation.getRequiredDataPoints(3)).toEqual(
            4,
        );
        expect(
            HermitePolynomialApproximation.getRequiredDataPoints(3, 1),
        ).toEqual(2);
        expect(
            HermitePolynomialApproximation.getRequiredDataPoints(5, 1),
        ).toEqual(3);
        expect(
            HermitePolynomialApproximation.getRequiredDataPoints(7, 1),
        ).toEqual(4);
    });
    ...
    it("higher order interpolation produces correct results.", function () {
        ...
        const expectedResult = [
            13367002.870928625, 0.0, -222.65168787012135, 0.0,
        ];
