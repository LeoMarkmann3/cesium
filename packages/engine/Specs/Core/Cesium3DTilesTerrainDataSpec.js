...
describe("Core/Cesium3DTilesTerrainData", function () {
    ...
    describe("upsample", function () {
        ...
        it("works for a quad with an extra vertex on the splitting plane", function () {
            ...
            const sc = lerpCartographic(sw, se, 0.5);
            const nc = lerpCartographic(nw, ne, 0.5);
            const centerBary = new Cartesian3(
                0.166666666,
                0.166666666,
                0.666666666,
            );
            ...
            return checkUpsampledTerrainData({
                ...
                nwIndices: new Uint16Array([
                    0, 5, 1, 1, 3, 2, 1, 5, 3, 3, 5, 4,
                ]),
                nwEdgeIndicesWest: new Uint16Array([0, 1]),
                nwEdgeIndicesSouth: new Uint16Array([4, 5, 0]),
                nwEdgeIndicesEast: new Uint16Array([2, 3, 4]),
                ...
                neIndices: new Uint16Array([
                    0, 5, 1, 1, 3, 2, 1, 4, 3, 1, 5, 4,
                ]),
                neEdgeIndicesWest: new Uint16Array([0, 1, 2]),
        ...
        it("upsamples an upsampled tile", async function () {
            ...
            const doublyUpsampleTerrainData =
                await upsampledTerrainData.upsample(
                    tilingScheme,
                    upsampledTileX,
                    upsampledTileY,
                    upsampledTileLevel,
            ...
            expect(doublyUpsampleTerrainData.wasCreatedByUpsampling()).toBe(
                true,
            );
            const doublyUpsampledMesh =
                await doublyUpsampleTerrainData.createMesh({
                    tilingScheme: tilingScheme,
    ...
    describe("interpolateHeight", function () {
        ...
        it("returns a height interpolated from the correct triangle", async function () {
            ...
            interpolatedHeight = data.interpolateHeight(
                rect,
                longitude,
                latitude,
            );
            expect(interpolatedHeight).toEqualEpsilon(
                height,
                CesiumMath.EPSILON3,
            );
    ...
    describe("isChildAvailable", function () {
        ...
        it("returns true for all children when child mask is not explicitly specified", function () {
            ...
            expect(data.isChildAvailable(x, y, x * 2 + 0, y * 2 + 0)).toBe(
                true,
            );
            expect(data.isChildAvailable(x, y, x * 2 + 1, y * 2 + 0)).toBe(
                true,
            );
            expect(data.isChildAvailable(x, y, x * 2 + 0, y * 2 + 1)).toBe(
                true,
            );
            expect(data.isChildAvailable(x, y, x * 2 + 1, y * 2 + 1)).toBe(
                true,
            );
        });
