...
describe("Core/Matrix2", function () {
    ...
    it("fromColumnMajorArray works with a result parameter", function () {
        const expected = new Matrix2(1.0, 2.0, 3.0, 4.0);
        const result = new Matrix2();
        const matrix = Matrix2.fromColumnMajorArray(
            [1.0, 3.0, 2.0, 4.0],
            result,
        );
        expect(matrix).toBe(result);
    ...
    it("fromScale works with a result parameter", function () {
        const expected = new Matrix2(7.0, 0.0, 0.0, 8.0);
        const result = new Matrix2();
        const returnedResult = Matrix2.fromScale(
    ...
    it("getColumn works", function () {
        ...
        const returnedResultColumn0 = Matrix2.getColumn(
            matrix,
            0,
            resultColumn0,
        );
        const returnedResultColumn1 = Matrix2.getColumn(
            matrix,
            1,
            resultColumn1,
        );
    ...
    it("setUniformScale works", function () {
        ...
        const returnedResult = Matrix2.setUniformScale(
            matrix,
            newScale,
            result,
        );
    ...
    it("getScale works", function () {
        ...
        const computedScale = Matrix2.getScale(
    ...
    it("setRotation works", function () {
        ...
        const scaleRotation = Matrix2.setRotation(
            scale,
            rotation,
            new Matrix2(),
        );

        const extractedScale = Matrix2.getScale(
            scaleRotation,
            new Cartesian2(),
        );
        const extractedRotation = Matrix2.getRotation(
            scaleRotation,
            new Matrix2(),
        );
    ...
    it("equalsEpsilon works with undefined", function () {
        ...
        expect(Matrix2.equalsEpsilon(new Matrix2(), undefined, 1.0)).toEqual(
            false,
        );
        expect(Matrix2.equalsEpsilon(undefined, new Matrix2(), 1.0)).toEqual(
            false,
        );
    });
