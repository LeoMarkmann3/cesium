...
describe("Core/EllipsoidRhumbLine", function () {
    ...
    it("computes heading for vertical lines", function () {
        ...
        expect(CesiumMath.PI).toEqualEpsilon(
            rhumb2.heading,
            CesiumMath.EPSILON12,
        );
    });
    ...
    it("computes distance at same latitude", function () {
        ...
        const rhumb = new EllipsoidRhumbLine(start, end, ellipsoid);
        const distance =
            Math.cos(fortyfiveDegrees) * CesiumMath.PI_OVER_TWO * 6;
        expect(distance).toEqualEpsilon(
            rhumb.surfaceDistance,
            CesiumMath.EPSILON12,
        );
    ...
    it("computes heading and distance given endpoints on sphere ", function () {
        ...
        expect(fifteenDegrees).toEqualEpsilon(
            rhumb2.heading,
            CesiumMath.EPSILON12,
        );
        expect(distance).toEqualEpsilon(
    ...
    it("tests sphere close to 90 degrees", function () {
        ...
        expect(rhumb1.heading).toEqualEpsilon(
            rhumb2.heading,
            CesiumMath.EPSILON12,
        );
    ...
    it("test across IDL with -PI to PI range of longitude", function () {
        ...
        expect(rhumb3.heading).toEqualEpsilon(
            rhumb4.heading,
            CesiumMath.EPSILON12,
        );
    ...
    it("test interpolate distance", function () {
        ...
        const interpolatedPoint =
            rhumb.interpolateUsingSurfaceDistance(distance);
    ...
    it("interpolates start and end points", function () {
        ...
        expect(start.latitude).toEqualEpsilon(
            first.latitude,
            CesiumMath.EPSILON12,
        );
        expect(end.longitude).toEqualEpsilon(
            last.longitude,
            CesiumMath.EPSILON12,
        );
        expect(end.latitude).toEqualEpsilon(
            last.latitude,
            CesiumMath.EPSILON12,
        );
    });
    ...
    it("interpolates when heading is near 0 degrees", function () {
        ...
        const expectedMid = new Cartographic(
            -threeHundredDegrees,
            thirtyDegrees,
        );
    ...
    it("interpolates midpoint using result parameter", function () {
        ...
        const result = new Cartographic();
        const midpoint = rhumb.interpolateUsingSurfaceDistance(
            distance,
            result,
        );
