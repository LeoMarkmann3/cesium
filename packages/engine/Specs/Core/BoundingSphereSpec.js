...
describe("Core/BoundingSphere", function () {
    ...
    it("equals", function () {
        ...
        expect(
            sphere.equals(
                new BoundingSphere(new Cartesian3(5.0, 2.0, 3.0), 4.0),
            ),
        ).toEqual(false);
        expect(
            sphere.equals(
                new BoundingSphere(new Cartesian3(1.0, 6.0, 3.0), 4.0),
            ),
        ).toEqual(false);
        expect(
            sphere.equals(
                new BoundingSphere(new Cartesian3(1.0, 2.0, 7.0), 4.0),
            ),
        ).toEqual(false);
        expect(
            sphere.equals(
                new BoundingSphere(new Cartesian3(1.0, 2.0, 3.0), 8.0),
            ),
        ).toEqual(false);
    ...
    it("fromPoints contains all points (naive)", function () {
        ...
        for (let i = 0; i < numPositions; i++) {
            ...
            expect(currentPos.x <= max.x && currentPos.x >= min.x).toEqual(
                true,
            );
            expect(currentPos.y <= max.y && currentPos.y >= min.y).toEqual(
                true,
            );
            expect(currentPos.z <= max.z && currentPos.z >= min.z).toEqual(
                true,
            );
        }
    });
    ...
    it("fromVertices contains all points (ritter)", function () {
        ...
        for (let i = 0; i < numElements; i += 3) {
            expect(positions[i] <= max.x && positions[i] >= min.x).toEqual(
                true,
            );
            expect(
                positions[i + 1] <= max.y && positions[i + 1] >= min.y,
            ).toEqual(true);
            expect(
                positions[i + 2] <= max.z && positions[i + 2] >= min.z,
            ).toEqual(true);
        }
    });
    ...
    it("fromRectangle2D", function () {
        ...
        const expected = new BoundingSphere(
            Cartesian3.ZERO,
            Math.sqrt(
                rectangle.east * rectangle.east +
                    rectangle.north * rectangle.north,
            ),
        );
    ...
    it("fromTransformation works with a result parameter", function () {
        ...
        const sphere = new BoundingSphere();
        const result = BoundingSphere.fromTransformation(
            transformation,
            sphere,
        );
    ...
    it("fromTransformation works without a result parameter", function () {
        ...
        expect(sphere.radius).toEqualEpsilon(
            expectedRadius,
            CesiumMath.EPSILON14,
        );
    });
    ...
    it("computes the volume of a BoundingSphere", function () {
        ...
        expect(computedVolume).toEqualEpsilon(
            expectedVolume,
            CesiumMath.EPSILON6,
        );
    });
