    ...
    function () {
        ...
        it("calling update updates data sources", function () {
            ...
            return Promise.all([
                ...
                display.update(Iso8601.MINIMUM_VALUE);
                expect(source1Visualizer.lastUpdateTime).toEqual(
                    Iso8601.MINIMUM_VALUE,
                );
                expect(source1Visualizer.updatesCalled).toEqual(1);
                expect(source2Visualizer.lastUpdateTime).toEqual(
                    Iso8601.MINIMUM_VALUE,
                );
        ...
        it("ready is true once datasources are ready and stays true", async function () {
            ...
            expect(display.ready)
                .withContext("before adding sources")
                .toBe(false);
            ...
            display.update(Iso8601.MINIMUM_VALUE);
            expect(display.ready)
                .withContext("after adding sources")
                .toBe(true);

            spyOn(MockVisualizer.prototype, "update").and.returnValue(false);
            display.update(Iso8601.MINIMUM_VALUE);
            expect(display.ready)
                .withContext("after updating again")
                .toBe(true);
        });
        ...
        it("sets dataSource primitives on add", function () {
            ...
            return dataSourceCollection.add(source).then(function () {
                ...
                expect(display._primitives.contains(source._primitives)).toBe(
                    true,
                );
                expect(
                    display._groundPrimitives.contains(
                        source._groundPrimitives,
                    ),
                ).toBe(true);
            });
        });
        ...
        it("adds primitives to scene when dataSource is added to the collection", function () {
            ...
            expect(
                scene.groundPrimitives.contains(display._groundPrimitives),
            ).toBe(false);

            return dataSourceCollection
                .add(new MockDataSource())
                .then(function () {
                    expect(scene.primitives.contains(display._primitives)).toBe(
                        true,
                    );
                    expect(
                        scene.groundPrimitives.contains(
                            display._groundPrimitives,
                        ),
                    ).toBe(true);
                });
        });
