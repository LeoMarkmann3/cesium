...
describe("DataSources/KmlTourFlyTo", function () {
    ...
    it("calls camera flyTo for KmlCamera", function () {
        ...
        return pollToPromise(function () {
            ...
            expect(
                fakeCamera.flyTo.calls.mostRecent().args[0].destination,
            ).toBe(position);
            expect(
                fakeCamera.flyTo.calls.mostRecent().args[0].orientation,
            ).toBe(hpr);
    ...
    it("calls camera flyToBoundingSphere for KmlLookAt", function () {
        ...
        return pollToPromise(function () {
            ...
            expect(
                fakeCamera.flyToBoundingSphere.calls.mostRecent().args[0].center
                    .x,
            ).toEqual(position.x);
            expect(
                fakeCamera.flyToBoundingSphere.calls.mostRecent().args[0].center
            ...
            expect(
                fakeCamera.flyToBoundingSphere.calls.mostRecent().args[1]
                    .offset,
            ).toBe(hpr);
            expect(doneSpy).toHaveBeenCalled();
        });
