...
describe("DataSources/StaticGroundGeometryPerMaterialBatch", function () {
    ...
    it("handles shared material being invalidated with geometry", function () {
        ...
        const geometryVisualizer = new GeometryVisualizer(
            scene,
            entityCollection,
        );
    ...
    it("updates with sampled distance display condition out of range", function () {
        ...
        const outOfRangeTime = JulianDate.fromIso8601(
            "2018-02-14T04:20:00+1100",
        );
        ...
        const entity = new Entity({
            availability: new TimeIntervalCollection([
                TimeInterval.fromIso8601({
                    iso8601:
                        "2018-02-14T04:00:00+1100/2018-02-14T04:30:00+1100",
                }),
            ]),
            position: new Cartesian3(1234, 5678, 9101112),
            ellipse: {
        ...
        return pollToPromise(function () {
            ...
            expect(attributes.distanceDisplayCondition).toEqual([
                0.0,
                Infinity,
            ]);
    ...
    it("has correct show attribute after rebuilding primitive", function () {
        ...
        return pollToPromise(renderScene)
            .then(function () {
                ...
                const primitive = scene.groundPrimitives.get(0);
                const attributes =
                    primitive.getGeometryInstanceAttributes(entity1);
                expect(attributes.show).toEqual([1]);
    ...
    it("doesn't immediately remove an empty batch that may be reused before the next update", async function () {
        ...
        const materialBefore =
            scene.groundPrimitives.get(0).appearance.material;
