...
describe(
    ...
    function () {
        ...
        it("removes the listener from the entity collection when destroyed", function () {
            ...
            expect(
                entityCollection.collectionChanged.numberOfListeners,
            ).toEqual(1);
            visualizer.destroy();
            expect(
                entityCollection.collectionChanged.numberOfListeners,
        ...
        it("A PathGraphics causes a primitive to be created and updated.", function () {
            ...
            path.material.color = new ConstantProperty(
                new Color(0.8, 0.7, 0.6, 0.5),
            );
            ...
            expect(primitive.show).toEqual(
                testObject.path.show.getValue(updateTime),
            );
        ...
        it("Switches from inertial to fixed paths in 2D", function () {
            ...
            expect(fixedPolylineCollection.modelMatrix).toEqual(
                Matrix4.IDENTITY,
            );
        ...
        it("clear hides primitives.", function () {
            ...
            path.width = new ConstantProperty(12.5);
            path.outlineColor = new ConstantProperty(
                new Color(0.1, 0.2, 0.3, 0.4),
            );
        ...
        it("subSample works for reference properties", function () {
            ...
            const referenceProperty = new ReferenceProperty(
                entities,
                "target",
                ["position"],
            );
        ...
        it("subSample works for callback position properties", function () {
            ...
            const callback = function (time, result) {
                if (
                    JulianDate.lessThan(time, t1) ||
                    JulianDate.greaterThan(time, t2)
                ) {
                    return undefined;
                }
                if (result === undefined) {
        ...
        function CustomPositionProperty(innerProperty) {
            ...
            this.getValueInReferenceFrame = function (
                time,
                referenceFrame,
                result,
            ) {
        ...
        function createCompositeTest(useReferenceProperty) {
            ...
            const scaledProperty = new ScaledPositionProperty(
                referenceProperty,
            );
            ...
            if (useReferenceProperty) {
                const testReference =
                    entities.getOrCreateEntity("testReference");
                testReference.position = property;
                propertyToTest = new ReferenceProperty(
                    entities,
                    "testReference",
                    ["position"],
                );
