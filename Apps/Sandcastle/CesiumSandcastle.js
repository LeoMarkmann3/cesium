...
require({
    ...
    function showDocPopup() {
        ...
        if (docError && selectedText && selectedText.length < 50) {
            ...
            for (
                let i = 0, len = local.docTypes[lowerText].length;
                i < len;
                ++i
            ) {
    ...
    CodeMirror.commands.runCesium = function (cm) {
        ...
        if (
            bucketFrame.contentWindow.location.href.indexOf("bucket.html") > 0
        ) {
    ...
    function applyBucket() {
        if (
            ...
            if (
                ...
                let attributeMatch;
                while (
                    (attributeMatch = attributeRegex.exec(bodyAttributes)) !==
                    null
                ) {
    ...
    if (window.location.search) {
        queryObject = ioQuery.queryToObject(
            ...
            if (defined(queryObject.gist)) {
                dojoscript
                    ...
                    .then(function (data) {
                        ...
                        const html = defined(htmlFile)
                            ? htmlFile.content
                            : defaultHtml; // Use the default html for old gists
    ...
    window.addEventListener(
        ...
        function (e) {
            ...
            if (e.data === "reload") {
                ...
                if (
                    bucketDoc.body.getAttribute("data-sandcastle-loaded") !==
                    "yes"
                ) {
                    bucketDoc.body.setAttribute(
                        "data-sandcastle-loaded",
                        "yes",
                    );
                    logOutput.innerHTML = "";
                    numberOfNewConsoleMessages = 0;
    ...
    registry.byId("search").on("change", function () {
        ...
        if (searchTerm !== "") {
            ...
            for (let i = 0; i < gallery_demos.length; i++) {
                ...
                if (
                    searchRegExp.test(demoName) ||
                    searchRegExp.test(demo.code)
                ) {
    ...
    function requestDemo(name) {
        return xhr.get({
            ...
            handleAs: "text",
            error: function (error) {
                loadFromGallery(gallery_demos[hello_world_index]).then(
                    function () {
