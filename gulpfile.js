...
export async function runCoverage(options) {
    ...
    if (workspace) {
        ...
        files = [
            ...
            {
                pattern: "packages/engine/Source/ThirdParty/**",
                included: false,
            },
...
export async function test() {
    ...
    const config = await karma.config.parseConfig(
        karmaConfigFile,
        {
            ...
            logLevel: verbose
                ? karma.constants.LOG_INFO
                : karma.constants.LOG_ERROR,
            files: files,
            proxies: proxies,
            client: {
    ...
    return new Promise((resolve, reject) => {
        const server = new karma.Server(config, function doneCallback(
            exitCode,
        ) {
...
function generateTypeScriptDefinitions(
    ...
    if (importModules) {
        ...
        Object.keys(importModules).forEach((workspace) => {
            const workspaceModules = Array.from(
                importModules[workspace],
            ).filter((importModule) => source.indexOf(importModule) !== -1);
