                ...
                for (const key in pragmas) {
                    if (pragmas.hasOwnProperty(key)) {
                        source = source.replace(
                            constructRegex(key, pragmas[key]),
                            "",
                        );
                    }
                }
...
export async function bundleCesiumJs(options) {
    ...
    buildConfig.plugins = options.removePragmas
        ? [stripPragmaPlugin]
        : undefined;
...
export async function glslToJavaScript(minify, minifyStateFilePath, workspace) {
    ...
    await Promise.all(
        glslFiles.map(async function (glslFile) {
            ...
            if (
                glslFile.indexOf(
                    path.normalize(path.join(baseDir, "Functions")),
                ) === 0
            ) {
                builtinFunctions.push(baseName);
            } else if (
                glslFile.indexOf(
                    path.normalize(path.join(baseDir, "Constants")),
                ) === 0
            ) {
                builtinConstants.push(baseName);
            } else if (
                glslFile.indexOf(
                    path.normalize(path.join(baseDir, "Structs")),
                ) === 0
            ) {
...
export async function copyEngineAssets(destination) {
    ...
    await copyFiles(
        [
            ...
            "!packages/engine/Source/Widget/*.js",
        ],
...
export async function createIndexJs(workspace) {
    ...
    if (!workspaceSources) {
        throw new Error(
            `Unable to find source files for workspace: ${workspace}`,
        );
    }
