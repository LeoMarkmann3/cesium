        ...
        return function (util) {
            return {
                compare: function (actualPromise, message) {
                    ...
                    if (
                        !defined(actualPromise) ||
                        !defined(actualPromise.then)
                    ) {
                        throw new Error(
                            "Expected function to be called on a promise.",
                        );
                    }
...
function createDefaultMatchers(debug) {
    return {
        ...
        toDrillPickPrimitive: function (util) {
            return {
                compare: function (actual, expected, x, y, width, height) {
                    return drillPickPrimitiveEquals(
                        actual,
                        1,
                        x,
                        y,
        ...
        toSampleHeightAndCall: function (util) {
            return {
                compare: function (
                    ...
                    const scene = actual;
                    const results = scene.sampleHeight(
                        position,
                        objectsToExclude,
                        width,
                    );
