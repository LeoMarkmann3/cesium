...
function makeBufferViews(subtreeDescription, subtreeJson) {
    ...
    const parsedAvailability = {
        tileAvailability: parseAvailability(
        ...
        if (useLegacySchema) {
            subtreeJson.extensions = {
                "3DTILES_multiple_contents": {
                    contentAvailability: contentAvailabilityArray.map(
                        function (x) {
        ...
        if (tableProperties.hasOwnProperty(key)) {
            ...
            if (defined(stringOffsets)) {
                propertyJson[stringOffsetsKey] =
                    firstMetadataIndex + stringOffsets;
            }

            if (defined(arrayOffsets)) {
                propertyJson[arrayOffsetsKey] =
                    firstMetadataIndex + arrayOffsets;
            }
